...
import { initKeystore, isKeystoreInitialized, loadAccount, readKeystoreFile, keystorePath, passphraseSource, describeSecretSource, } from '../lib/agent-keystore.js';
import { loadPolicy, checkPolicy, policyPath, policyDigest, validatePolicyInput, savePolicy, } from '../lib/agent-policy.js';
import { POLICY_TEMPLATES, findTemplate } from '../lib/agent-policy-templates.js';
import { loadTrackedTokens, addTrackedToken, removeTrackedToken, tokensPath } from '../lib/agent-tokens.js';
...
import { loadCustomChains, addCustomChain, removeCustomChain, customChainsPath, resolveChainWithCustom, rpcEndpointsWithCustom, mergedChains, } from '../lib/agent-custom-chains.js';
import { BLUE_CHIPS } from '../lib/agent-blue-chips.js';
/**
...
export const agentWalletTools = [
    {
        name: 'chaingpt_agent_wallet_init',
        description: 'Initialize the agent\'s wallet — generates a fresh EOA, encrypts the private key with the passphrase ' +
    ...
    },
    {
        ...
        description: 'Return the agent\'s public EOA address (for receiving funds). Does NOT need the passphrase — reads ' +
    ...
    },
    {
        ...
        description: 'Show the agent wallet\'s full status: address, keystore path, current policy summary + digest, policy ' +
    ...
    },
    {
        ...
        description: 'Get the agent\'s native-coin balance across one or more EVM chains. Uses public-RPC fallbacks per chain. ' +
            '0 ChainGPT credits.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Display the current policy file content + digest. Read-only — this tool does NOT modify the policy. ' +
    ...
    },
    {
        ...
        description: 'Sign and broadcast an EVM transaction from the agent\'s wallet. Every call: loads the policy file ' +
    ...
    },
    {
        ...
        description: 'Start a local HTML dashboard on a localhost port. Read-only view: address, multi-chain balances, ' +
...
async function getNativeBalance(chainSlug, address) {
    const endpoints = rpcEndpointsWithCustom(chainSlug);
    if (endpoints.length === 0)
        throw new Error(`No RPC endpoints for ${chainSlug}`);
    const hex = await jsonRpcFallback(endpoints, 'eth_getBalance', [address, 'latest']);
    return BigInt(hex);
}
...
async function cachedBalance(key, fetchFn) {
    const now = Date.now();
...
let adminToken = null;
const sessions = new Map(); // sessionId -> expiry ms
...
`;
async function renderDashboard(res, address, chains, flash) {
    ...
    const nativeResults = await Promise.all(chains.map(async (c) => {
    ...
    for (const r of nativeResults) {
        ...
        nativeRows.push(`<div class="balance-row${zeroClass}">
        <div class="left">
    ...
    const tokenResults = await Promise.all(tracked.map(async (t) => {
    ...
    for (const r of tokenResults) {
        ...
        tokenRows.push(`<div class="balance-row${zeroClass}">
        <div class="left">
...
function renderTabbedDashboard(o) {
  ...
  `).join('');
    ...
    const allowedRows = (o.policy.allowedToAddresses ?? ['']).map((a) => `<div class="row"><input type="text" name="allowed" value="${escapeHtml(a)}" placeholder="0x..."/><button type="button" class="remove secondary remove-row">×</button></div>`).join('');
    const blockedRows = (o.policy.blockedToAddresses ?? ['']).map((a) => `<div class="row"><input type="text" name="blocked" value="${escapeHtml(a)}" placeholder="0x..."/><button type="button" class="remove secondary remove-row">×</button></div>`).join('');
    const selectorRows = (o.policy.blockedSelectors ?? ['']).map((s) => `<div class="row"><input type="text" name="selector" value="${escapeHtml(s)}" placeholder="0xa9059cbb"/><button type="button" class="remove secondary remove-row">×</button></div>`).join('');
    const maxValueWei = o.policy.maxTxValueWei ?? '0';
    // Token-add chain options — merged built-in + custom
    const tokenChainOptions = mergedChainList.map((c) => `<option value="${escapeHtml(c.slug)}">${escapeHtml(c.name)}${!CHAINS[c.slug] ? ' (custom)' : ''}</option>`).join('');
...
export async function handleAgentWalletTool(name, args) {
    args ??= {};
    try {
        // ── init ────────────────────────────────────────────────────────
        ...
        if (name === 'chaingpt_agent_wallet_status') {
            ...
            if (policy.allowedChains?.length)
                lines.push(`Allowed chains:  ${policy.allowedChains.join(', ')}`);
            if (policy.allowedToAddresses?.length)
                lines.push(`Allowed to:      ${policy.allowedToAddresses.length} addresses`);
            if (policy.blockedToAddresses?.length)
                lines.push(`Blocked to:      ${policy.blockedToAddresses.length} addresses`);
            if (policy.maxTxValueWei)
                lines.push(`Max value/tx:    ${policy.maxTxValueWei} wei (${fmtEth(BigInt(policy.maxTxValueWei))} native)`);
            if (policy.requireMemo)
                lines.push(`Memo required:   yes`);
            ...
            // Solana surface
            {
                ...
                if (!solFile) {
                    ...
                    if (solPolicy?.enabled) {
                        if (solPolicy.allowedPrograms)
                            lines.push(`  Programs:      ${solPolicy.allowedPrograms.length} allowed`);
                        if (solPolicy.maxTxLamports)
                            lines.push(`  Max/tx:        ${fmtSol(BigInt(solPolicy.maxTxLamports))} SOL`);
        ...
        if (name === 'chaingpt_agent_wallet_balances') {
            ...
            const chains = Array.isArray(args.chains) && args.chains.length > 0
                ? args.chains
                : ['ethereum', 'base', 'arbitrum'];
            const lines = [`Agent wallet balances — ${file.address}`, ''];
            for (const c of chains) {
                const chain = resolveChainWithCustom(c);
                if (!chain) {
                    lines.push(`  ${c}: unknown chain`);
                    continue;
                }
                try {
        ...
        if (name === 'chaingpt_agent_wallet_sign_and_send') {
            ...
            try {
                ...
                return { content: [{ type: 'text', text: `Keystore load failed: ${e?.message ?? e}` }] };
            }
            const endpoints = rpcEndpointsWithCustom(chainSlug);
            if (endpoints.length === 0) {
                ...
                try {
                    ...
                    if (method === 'POST' && url === '/api/policy/template') {
                        ...
                        if (!tmpl) {
                            if (wantsJson) {
                                ...
                                res.end(JSON.stringify({ ok: false, error: `Unknown template: ${fields.template}` }));
                                return;
                            }
                        ...
                        if (!validation.ok || !validation.policy) {
                            if (wantsJson) {
                                res.writeHead(400, { 'content-type': 'application/json' });
                                res.end(JSON.stringify({ ok: false, error: `Template invalid: ${validation.error}` }));
                                return;
                            }
                        ...
                        try {
                            ...
                            if (wantsJson) {
                                ...
                                res.end(JSON.stringify({ ok: false, error: `Save failed: ${e?.message ?? e}` }));
                                return;
                            }
                            renderDashboard(res, file.address, chains, { kind: 'err', msg: `Save failed: ${e?.message ?? e}` });
                        }
                    ...
                    if (method === 'POST' && url === '/api/scan-bluechips') {
                        ...
                        for (const [chain, tokens] of Object.entries(BLUE_CHIPS)) {
                            if (!resolveChainWithCustom(chain))
                                continue;
                            for (const tok of tokens) {
                                if (have.has(key(chain, tok.address)))
                                    continue;
                                try {
                    ...
                    if (url.startsWith('/api/status')) {
                        ...
                        for (const c of chains) {
                            try {
                                balances[c] = (await getNativeBalance(c, file.address)).toString();
                            }
                            catch {
                                balances[c] = 'error';
                            }
