...
import { CHAINS, rpcEndpoints } from '../lib/chains.js';
/**
...
function publicClientFor(network) {
...
const EIGEN_STRATEGY_MANAGER = '0x858646372CC42E1A627fcE94aa7A7033e7CF075A';
// ─── ABI fragments ──────────────────────────────────────────────────
const AAVE_POOL_ABI = [
    {
        type: 'function',
...
export const defiTools = [
    {
        ...
        description: 'Read an account\'s Aave V3 health-factor and position summary on a given chain. Returns total collateral, ' +
    ...
    },
    {
        ...
        description: 'Build an UNSIGNED Aave V3 supply transaction. Requires `acknowledgeMainnet: true`. The user must have ' +
    ...
    },
    {
        ...
        description: 'Build an UNSIGNED Aave V3 borrow transaction. Requires sufficient collateral + health factor. ' +
    ...
    },
    {
        ...
        description: 'Build an UNSIGNED Aave V3 repay transaction. Pass amount="max" for full repayment. ' +
            'Requires `acknowledgeMainnet: true`. 0 ChainGPT credits.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Build an UNSIGNED Aave V3 withdraw transaction. Pass amount="max" for full withdrawal of the supplied ' +
    ...
    },
    {
        ...
        description: 'Build an UNSIGNED Lido stETH staking transaction on Ethereum mainnet. Sends native ETH; receives stETH ' +
    ...
    },
    {
        ...
        description: 'Build an UNSIGNED EigenLayer deposit transaction (restake an LST like stETH / rETH / cbETH into a ' +
        ...
        inputSchema: {
            ...
            properties: {
                strategy: {
                    type: 'string',
                    description: 'EigenLayer Strategy address. Common: stETH=0x93c4b944D05dfe6df7645A86cd2206016c51564D, ' +
...
// ─── Helpers ────────────────────────────────────────────────────────
const MAINNET_REFUSAL = (action, network) => `⚠ Mainnet ${action} refused. To execute on ${CHAINS[network]?.name ?? network} mainnet, pass ` +
...
function formatTx(chainId, to, data, valueWei = 0n) {
    return {
        chainId,
        to,
        data,
...
export async function handleDefiTool(name, args) {
    if (!args) {
    ...
    try {
        if (name === 'chaingpt_defi_aave_health') {
            ...
            if (!user)
                return { content: [{ type: 'text', text: 'Error: user is required.' }] };
            const network = String(args.network ?? 'ethereum');
            const pool = AAVE_POOL[network];
            if (!pool)
                return { content: [{ type: 'text', text: `Aave V3 not deployed on ${network}.` }] };
        ...
        if (name === 'chaingpt_defi_aave_supply_tx') {
            ...
            const onBehalfOf = (args.onBehalfOf || from);
        ...
        if (name === 'chaingpt_defi_aave_repay_tx') {
            ...
            const amount = amountInput === 'max' ? maxUint() : parseUnits(amountInput, decimals);
            const from = String(args.from);
            const onBehalfOf = (args.onBehalfOf || from);
            const rateMode = BigInt(Number(args.interestRateMode ?? 2));
            const data = encodeFunctionData({
        ...
        if (name === 'chaingpt_defi_lido_stake_tx') {
            ...
            if (!amountEth)
                return { content: [{ type: 'text', text: 'amountEth is required.' }] };
