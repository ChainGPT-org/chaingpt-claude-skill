import { ALL_CHAIN_SLUGS, CHAINS, EVM_CHAIN_SLUGS, rpcEndpoints } from '../lib/chains.js';
...
export const walletTools = [
    {
        ...
        description: 'Get native + ERC-20 token balances for a wallet address across multiple chains. ' +
    ...
    },
    {
        ...
        description: 'Get DeFi positions (liquidity, lending, staking, farming) for a wallet across protocols. ' +
    ...
    },
    {
        ...
        description: 'Get profit/loss summary for a wallet on a given chain. Returns realized + unrealized P&L, ' +
...
async function fetchMoralisBalances(address, chainSlug, key) {
    const moralisChain = MORALIS_CHAIN_MAP[chainSlug];
    if (!moralisChain)
        return [];
    const url = `${MORALIS_BASE}/wallets/${address}/tokens?chain=${moralisChain}`;
    const res = await httpJson(url, { headers: moralisHeaders(key) });
    return res.result ?? [];
}
async function fetchNativeBalanceViaRpc(address, chainSlug) {
...
export async function handleWalletTool(name, args) {
    ...
    try {
        if (name === 'chaingpt_wallet_balances') {
            ...
            const minUsd = args.minUsdValue ?? 1;
            ...
            for (const slug of requestedChains) {
                ...
                const filtered = tokens.filter((t) => {
                    if (!includeNative && t.native_token)
                        return false;
                    if (t.usd_value !== undefined && t.usd_value < minUsd)
                        return false;
                    return true;
                });
                ...
                if (filtered.length === 0) {
                    ...
                    for (const t of filtered)
                        sections.push(formatTokenLine(t));
                }
            }
            sections.push('');
        ...
        if (name === 'chaingpt_wallet_positions') {
            ...
            if (!key) {
                return {
                    content: [{
                            type: 'text',
                            text: 'MORALIS_API_KEY is required for chaingpt_wallet_positions.\n' +
