import { withRpcFallback, parseAddress, buildVersionedTransaction, serializeUnsigned, deserializeUnsigned, buildSolTransferInstruction, buildSplTransferCheckedInstruction, buildCreateAtaIdempotentInstruction, deriveAssociatedTokenAccount, fetchMintInfo, isMainnet, TOKEN_PROGRAM_ID, TOKEN_2022_PROGRAM_ID, } from '../lib/solana-sign.js';
/**
...
export const solanaTools = [
    {
        ...
        description: 'Build an UNSIGNED Solana transfer transaction (custody-free). Handles native SOL transfers ' +
        ...
        inputSchema: {
            ...
            properties: {
                from: {
                    type: 'string',
                    description: 'Base58 sender address. This is the wallet that will sign the transaction and pay fees. ' +
                        'For SPL transfers, this is the OWNER address (not the ATA — the ATA is derived).',
                },
                to: {
                    type: 'string',
                    description: 'Base58 recipient address (owner address, not ATA). For SPL transfers, the recipient ATA ' +
                        'is derived; if it does not exist, an idempotent create-ATA instruction is included.',
                },
                amount: {
                    type: 'string',
                    description: 'Human-readable amount as a string (e.g. "1.5" for 1.5 SOL or 1.5 USDC). Decimals are ' +
                        'fetched from the mint for SPL transfers, or LAMPORTS_PER_SOL (9) for native SOL.',
                },
                mint: {
                    type: 'string',
                    description: 'Optional. Base58 SPL token mint address. Omit for native SOL. Examples: USDC mainnet = ' +
                ...
                acknowledgeMainnet: {
                    type: 'boolean',
                    description: 'Must be true when network=mainnet. Prevents accidental mainnet tx construction. ' +
    ...
    },
    {
        ...
        description: 'Decode an unsigned base64-encoded Solana VersionedTransaction for human review. Returns the ' +
...
function decimalToBaseUnits(amount, decimals) {
    ...
    if (frac.length > decimals) {
        throw new Error(`amount "${amount}" has ${frac.length} fractional digits but mint has only ${decimals} decimals`);
    }
    const padded = (frac + '0'.repeat(decimals - frac.length));
    return BigInt(whole + padded);
}
async function handleBuildTransferTx(args) {
    ...
    if (!mintArg) {
        ...
        summary.push(`Native SOL transfer`, `  from:    ${from.toBase58()}`, `  to:      ${to.toBase58()}`, `  amount:  ${amount} SOL (${lamports.toString()} lamports)`);
    }
    else {
        ...
        summary.push(`${tokenStandard} transfer`, `  mint:        ${mint.toBase58()} (decimals=${mintInfo.decimals})`, `  from owner:  ${from.toBase58()}`, `  from ATA:    ${sourceAta.toBase58()}`, `  to owner:    ${to.toBase58()}`, `  to ATA:      ${destAta.toBase58()}  (create-if-missing instruction included)`, `  amount:      ${amount} (${baseUnits.toString()} base units)`);
    }
    const { tx, blockhash, lastValidBlockHeight } = await withRpcFallback(network, (c) => buildVersionedTransaction({ payer: from, instructions, connection: c }));
...
async function handleDecodeTx(args) {
