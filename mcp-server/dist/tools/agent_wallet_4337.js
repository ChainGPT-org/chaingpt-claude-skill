...
import { loadPolicy, policyPath, checkPolicy, checkErc4337Gate, } from '../lib/agent-policy.js';
import { logActivity, spendStats } from '../lib/agent-activity.js';
import { encodeSingleExecute, encodeGetNonce, nexusNonceKey, readAccountId, classifyAccountId, } from '../lib/erc7579.js';
import { SMART_SESSIONS_ADDRESS, encodeUseSignature, MOCK_ECDSA_SIG } from '../lib/smart-sessions.js';
import { ENTRY_POINT_V07, normalizeUserOp, computeUserOpHash, userOpToBundlerJson, bundlerRpc, } from '../lib/erc4337.js';
 ...
 */
export const agentWallet4337Tools = [
    {
        name: 'chaingpt_agent_wallet_4337_sign_and_send',
        description: "Execute one call through the user's ERC-7579 smart account using the agent's on-chain session key: " +
...
export async function handleAgentWallet4337Tool(name, args) {
    args = args ?? {};
    try {
        ...
        if (!gate.allowed)
            return refusalBlock(gate.reason, gate.policyDigest);
        ...
        const nonceRaw = await jsonRpcFallback(rpcs, 'eth_call', [
            { to: ENTRY_POINT_V07, data: encodeGetNonce(account, nexusNonceKey(SMART_SESSIONS_ADDRESS)) },
            'latest',
        ]);
        const nonce = decodeAbiParameters([{ type: 'uint256' }], nonceRaw)[0];
        const feeHex = await jsonRpcFallback(rpcs, 'eth_gasPrice', []);
        ...
        try {
            const est = await bundlerRpc({
                url: bundlerUrl,
                method: 'eth_estimateUserOperationGas',
                params: [userOpToBundlerJson(normalizeUserOp(baseOp)), ENTRY_POINT_V07],
            });
            const bump = (h) => `0x${((BigInt(h) * BigInt(headroomPct)) / 100n).toString(16)}`;
            ...
            return {
                content: [{
                        type: 'text',
                        text: `Bundler refused gas estimation: ${e?.message ?? e}\n\n` +
        ...
        if (args.waitForReceipt !== false) {
            for (let i = 0; i < 20 && !txHash; i++) {
                try {
                    const r = await bundlerRpc({ url: bundlerUrl, method: 'eth_getUserOperationReceipt', params: [submittedHash] });
