import { normalizeUserOp, computeUserOpHash, packUserOp, userOpToBundlerJson, bundlerRpc, ENTRY_POINT_V07, } from '../lib/erc4337.js';
...
export const aaTools = [
    {
        name: 'chaingpt_aa_userop_hash',
        description: 'Compute the ERC-4337 v0.7 userOpHash — the digest that a smart-contract wallet\'s owner or ' +
    ...
    },
    {
        ...
        description: 'Pack a v0.7 UserOperation into the wire-format `PackedUserOperation` struct that the ' +
    ...
    },
    {
        ...
        description: 'Call eth_estimateUserOperationGas on a bundler RPC. The admin supplies the bundler URL ' +
    ...
    },
    {
        ...
        description: 'Call eth_getUserOperationReceipt on a bundler RPC. Returns the receipt (tx hash, status, ' +
    ...
    },
    {
        ...
        description: 'Submit a SIGNED userOperation to a bundler (eth_sendUserOperation). Custody-free: refuses when ' +
...
function chainIdFor(slug) {
    ...
    if (!c)
        throw new Error(`chain: "${slug}" not supported. Use one of: ${Object.keys(CHAINS).join(', ')} (or register a custom chain like base-sepolia).`);
    if (c.chainId === null)
        throw new Error(`chain: "${slug}" has no numeric chainId (non-EVM); ERC-4337 only applies to EVM chains`);
    return c.chainId;
}
async function handleUserOpHash(args) {
    const userOp = normalizeUserOp(args.userOp);
...
async function handleUserOpPack(args) {
    const userOp = normalizeUserOp(args.userOp);
    const chainId = chainIdFor(args.chain);
    const entryPoint = args.entryPoint ?? ENTRY_POINT_V07;
...
async function handleEstimateUserOp(args) {
...
async function handleSubmitUserOp(args) {
    const userOp = normalizeUserOp(args.userOp);
    if (!userOp.signature || userOp.signature === '0x') {
        throw new Error('Refused: userOp.signature is empty. This tool is custody-free — compute the hash with ' +
...
async function handleUserOpReceipt(args) {
    if (typeof args.bundlerUrl !== 'string' || !args.bundlerUrl.startsWith('http')) {
        throw new Error('bundlerUrl required (https://… your bundler RPC)');
    }
...
export async function handleAaTool(name, args) {
    let text;
    try {
        if (name === 'chaingpt_aa_userop_hash')
            text = await handleUserOpHash(args);
        else if (name === 'chaingpt_aa_pack_userop')
            text = await handleUserOpPack(args);
        else if (name === 'chaingpt_aa_estimate_userop')
            text = await handleEstimateUserOp(args);
        else if (name === 'chaingpt_aa_userop_receipt')
            text = await handleUserOpReceipt(args);
        else if (name === 'chaingpt_aa_submit_userop')
            text = await handleSubmitUserOp(args);
        else
            throw new Error(`Unknown AA tool: ${name}`);
    }
    catch (err) {
