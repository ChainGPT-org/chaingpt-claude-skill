 ...
 */
export const marginfiSignedTools = [
    {
        name: 'chaingpt_defi_marginfi_deposit_tx',
        description: 'Build an UNSIGNED Marginfi v2 deposit transaction (Solana mainnet, custody-free). Lends a token into a ' +
    ...
    },
    {
        ...
        description: 'Build an UNSIGNED Marginfi v2 withdraw transaction (Solana mainnet, custody-free). Withdraws a previously ' +
...
async function buildMarginfiAction(kind, args) {
    ...
    if (!args.symbol && !args.mint)
        throw new Error('Provide either symbol (e.g. "USDC") or mint to identify the bank.');
    ...
    const wantMint = args.mint;
    const matched = Object.entries(metadata).find(([, m]) => wantMint ? m.tokenAddress === wantMint : (m.tokenSymbol ?? '').toUpperCase() === wantSymbol);
    if (!matched) {
        throw new Error(`No Marginfi bank found for ${wantMint ? `mint ${wantMint}` : `symbol "${args.symbol}"`}. ` +
    ...
    if (!bank && wantMint)
        bank = client.getBankByMint(wantMint);
    if (!bank && args.symbol)
        bank = client.getBankByTokenSymbol(args.symbol);
    if (!bank) {
    ...
    if (accounts.length === 0) {
        throw new Error(`No Marginfi account exists for ${user.toBase58()}. Create one first (deposit once via app.marginfi.com, ` +
            `or a future version will prepend an account-creation instruction). This tool deposits into an existing account.`);
    }
    const account = accounts[0];
    let instructions;
    if (kind === 'deposit') {
        const wrapper = await account.makeDepositIx(amount, bank.address);
        instructions = wrapper.instructions;
    }
    else {
        const wrapper = await account.makeWithdrawIx(withdrawAll ? (amount ?? '0') : amount, bank.address, withdrawAll);
        instructions = wrapper.instructions;
    }
    ...
    try {
        ...
        simText = formatSimResult(sim, 'Marginfi');
    }
    catch (e) {
        simText = `Simulation: could not run (${e?.message ?? e}). Decode + review the tx manually before signing.`;
    }
    const tokenLabel = bank.tokenSymbol || bank.meta?.tokenSymbol || args.symbol || args.mint;
    return [
...
export async function handleMarginfiSignedTool(name, args) {
    let text;
    const safeArgs = args ?? {};
    try {
        if (name === 'chaingpt_defi_marginfi_deposit_tx')
            text = await buildMarginfiAction('deposit', safeArgs);
        else if (name === 'chaingpt_defi_marginfi_withdraw_tx')
            text = await buildMarginfiAction('withdraw', safeArgs);
        else
            throw new Error(`Unknown Marginfi signed tool: ${name}`);
    }
    catch (e) {
