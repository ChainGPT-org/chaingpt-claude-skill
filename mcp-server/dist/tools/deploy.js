import { createPublicClient, http, encodeAbiParameters, formatEther, formatGwei, parseEther, } from 'viem';
import { mainnet, base, arbitrum, optimism, polygon, bsc, avalanche, blast, linea, scroll, sepolia, baseSepolia, arbitrumSepolia, optimismSepolia, polygonAmoy, bscTestnet, } from 'viem/chains';
...
function viemClient(network) {
    const chain = NETWORK_TO_VIEM[network];
    if (!chain)
...
export const deployTools = [
    {
        ...
        description: 'Compile Solidity source code (single file or multi-contract) using solc 0.8.x. Returns bytecode, ABI, ' +
    ...
    },
    {
        ...
        description: 'Estimate the gas cost (in gas units + USD) of deploying a contract on a given network. MAINNET is ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                constructorAbi: {
                    description: 'Constructor ABI fragment from the compiled ABI (the entry with type="constructor"). Optional ' +
    ...
    },
    {
        ...
        description: 'Build an UNSIGNED deployment transaction object for the user\'s wallet to sign + broadcast. ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                acknowledgeMainnet: {
                    type: 'boolean',
                    description: 'You must pass acknowledgeMainnet=true to build a tx targeting a mainnet network. ' +
    ...
    },
    {
        ...
        description: 'Submit verified-source publication to Etherscan v2 for an already-deployed contract. Works across all ' +
    ...
    },
    {
        ...
        description: 'Poll the status of a verification submission. Returns success/pending/failed plus the explorer link ' +
...
function chainNameForNetwork(network) {
...
export async function handleDeployTool(name, args) {
    ...
    try {
        if (name === 'chaingpt_deploy_compile') {
            ...
            const ctorEntry = result.abi.find((x) => x.type === 'constructor');
            const lines = [
                ...
                ctorEntry ? `Constructor args:  ${(ctorEntry.inputs ?? []).map((i) => `${i.type} ${i.name}`).join(', ') || 'none'}` : 'Constructor:       none',
        ...
        if (name === 'chaingpt_deploy_estimate' || name === 'chaingpt_deploy_build_tx') {
            ...
            if (name === 'chaingpt_deploy_build_tx' && isMainnet && args.acknowledgeMainnet !== true) {
                return {
                    content: [{
                            type: 'text',
                            text: `⚠ Mainnet deploy refused. To deploy to ${chainNameForNetwork(network)} (chainId ${NETWORK_TO_CHAIN_ID[network]}), ` +
            ...
            let data = bytecode;
            const constructorAbi = args.constructorAbi;
            ...
            try {
                ...
                return {
                    content: [{
                            type: 'text',
                            text: `Gas estimate failed on ${chainNameForNetwork(network)}: ${msg}\n` +
        ...
        if (name === 'chaingpt_deploy_verify') {
            ...
            return {
                content: [{
                        type: 'text',
                        text: `Verification submitted for ${address} on ${chainNameForNetwork(network)}.\n` +
                            `GUID: ${data.result}\n\n` +
        ...
        if (name === 'chaingpt_deploy_verify_status') {
            ...
            if (!chainId)
                return { content: [{ type: 'text', text: `Unknown network: ${network}` }] };
            const url = `${ETHERSCAN_V2_BASE}?chainid=${chainId}&module=contract&action=checkverifystatus` +
