...
export const chatTools = [
    {
        name: 'chaingpt_chat',
        description: 'Send a question to ChainGPT Web3 AI LLM. Returns crypto/blockchain answers with live on-chain data including token prices, wallet analysis, whale tracking, DeFi positions, and more. Costs 0.5 credits (1.0 with chat history).',
        inputSchema: {
            type: 'object',
            properties: {
                question: {
                ...
                sessionId: {
                    type: 'string',
                    description: 'Unique session ID for conversation continuity (required when chatHistory is true)',
                },
            },
            required: ['question'],
        },
    },
    {
        ...
        description: 'Chat with ChainGPT using custom context injection — the AI will answer as if it is your project\'s assistant, with knowledge of your company, token, and branding. Costs 0.5 credits (1.0 with chat history).',
        inputSchema: {
            type: 'object',
            properties: {
                question: {
                ...
                blockchain: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'Blockchain networks the token is on. Values: ETHEREUM, BSC, ARBITRUM, BASE, BLAST, AVALANCHE, POLYGON, SCROLL, OPTIMISM, LINEA, ZKSYNC, POLYGON_ZKEVM, GNOSIS, FANTOM, MOONRIVER, MOONBEAM, BOBA, METIS, LISK, AURORA, SEI, IMMUTABLE_ZK, GRAVITY, TAIKO, CRONOS, FRAXTAL, ABSTRACT, WORLD_CHAIN, MANTLE, MODE, CELO, BERACHAIN',
                },
                tone: {
                    type: 'string',
                ...
                customTone: {
                    type: 'string',
                    description: 'Custom tone instructions when tone is not a preset. Provide free-text describing the desired AI personality.',
                },
                limitation: {
                    type: 'boolean',
                    description: 'If true, the AI will only answer questions related to the injected context and refuse off-topic queries.',
                    default: false,
                },
                chatHistory: {
                    type: 'boolean',
    ...
    },
    {
        ...
        description: 'Retrieve chat conversation history for a given session. Requires a session ID that was used in previous chat calls.',
        inputSchema: {
            type: 'object',
            properties: {
                sessionId: {
...
export async function handleChatTool(name, args) {
    ...
    try {
        if (name === 'chaingpt_chat') {
            ...
            const botResponse = response?.data?.bot ?? JSON.stringify(response);
            return { content: [{ type: 'text', text: botResponse }] };
        }
        if (name === 'chaingpt_chat_with_context') {
            const contextInjection = {};
            if (args.companyName)
                contextInjection.companyName = args.companyName;
            if (args.companyDescription)
                contextInjection.companyDescription = args.companyDescription;
            if (args.companyWebsiteUrl)
                contextInjection.companyWebsiteUrl = args.companyWebsiteUrl;
            if (args.tokenName || args.tokenSymbol || args.tokenAddress) {
                contextInjection.cryptoToken = true;
                const tokenInformation = {};
                if (args.tokenName)
                    tokenInformation.tokenName = args.tokenName;
                if (args.tokenSymbol)
                    tokenInformation.tokenSymbol = args.tokenSymbol;
                if (args.tokenAddress)
                    tokenInformation.tokenAddress = args.tokenAddress;
                if (args.blockchain)
                    tokenInformation.blockchain = args.blockchain;
                contextInjection.tokenInformation = tokenInformation;
            }
            if (args.whitePaperUrl)
                contextInjection.whitePaperUrl = args.whitePaperUrl;
            if (args.purpose)
                contextInjection.purpose = args.purpose;
            if (args.tokenSourceCode || args.tokenAuditUrl || args.exploreUrl || args.cmcUrl || args.coingeckoUrl) {
                const tokenInfo = contextInjection.tokenInformation || {};
                if (args.tokenSourceCode)
                    tokenInfo.tokenSourceCode = args.tokenSourceCode;
                if (args.tokenAuditUrl)
                    tokenInfo.tokenAuditUrl = args.tokenAuditUrl;
                if (args.exploreUrl)
                    tokenInfo.exploreUrl = args.exploreUrl;
                if (args.cmcUrl)
                    tokenInfo.cmcUrl = args.cmcUrl;
                if (args.coingeckoUrl)
                    tokenInfo.coingeckoUrl = args.coingeckoUrl;
                contextInjection.tokenInformation = tokenInfo;
                if (!contextInjection.cryptoToken)
                    contextInjection.cryptoToken = true;
            }
