...
export const yieldTools = [
    // ─── Pendle ───────────────────────────────────────────────────────
    {
        ...
        description: 'List active Pendle yield-trading markets on a chain. Each market lets you split a yield-bearing asset ' +
    ...
    },
    {
        ...
        description: 'Get a single Pendle market in detail: maturity, current PT/YT prices, fixed vs implied APY spread, ' +
    ...
    // ─── Morpho Blue ───────────────────────────────────────────────────
    {
        ...
        description: 'List active Morpho Blue lending markets. Each market is a (loan_token, collateral_token, oracle, ' +
    ...
    },
    {
        ...
        description: 'List MetaMorpho vaults — curated baskets of Morpho Blue markets run by professional risk curators ' +
    ...
    },
    {
        ...
        description: "Get a user's Morpho Blue positions: which markets they're lending/borrowing in, sizes, health factors. " +
...
async function morphoGql(query, variables = {}) {
    ...
    if (!res.data)
        throw new Error('Morpho GraphQL returned no data');
    return res.data;
}
// Morpho chain ids
const MORPHO_CHAIN_IDS = {
    ethereum: 1,
    base: 8453,
};
export async function handleYieldTool(name, args) {
    if (!args)
    ...
    try {
        ...
        if (name === 'chaingpt_defi_pendle_markets') {
            ...
            const res = await httpJson(`${PENDLE_API}/v1/${chain.chainId}/markets/active`);
            ...
            const liqOf = (m) => Number(m?.details?.liquidity ?? m?.liquidity?.usd ?? 0);
            markets.sort((a, b) => liqOf(b) - liqOf(a));
        ...
        if (name === 'chaingpt_defi_pendle_market') {
            ...
            const res = await httpJson(`${PENDLE_API}/v1/${chain.chainId}/markets/${addr}`);
            ...
            lines.push('');
            if (res?.pt)
                lines.push(`PT (Principal Token):  ${res.pt.address}    symbol: ${res.pt.symbol ?? '?'}`);
            if (res?.yt)
                lines.push(`YT (Yield Token):      ${res.yt.address}    symbol: ${res.yt.symbol ?? '?'}`);
            if (res?.sy)
                lines.push(`SY (Standardized Yield): ${res.sy.address}    symbol: ${res.sy.symbol ?? '?'}`);
            lines.push('');
        ...
        markets(first: $first, where: $where, orderBy: SupplyAssetsUsd, orderDirection: Desc) {
          items {
            ...
            const data = await morphoGql(query, { first: limit, where: { chainId_in: [chainId], listed: true } });
            const items = data?.markets?.items ?? [];
            const lines = [];
            lines.push(`Morpho Blue markets — ${network} (top ${items.length} by supply TVL)`);
            lines.push('');
            if (items.length === 0)
                lines.push('(no markets)');
        ...
        vaults(first: $first, where: $where, orderBy: TotalAssetsUsd, orderDirection: Desc) {
          items {
            ...
            const where = { chainId_in: [chainId] };
            if (assetSym)
                where.assetSymbol_in = [assetSym];
            const data = await morphoGql(query, { first: limit, where });
            const items = data?.vaults?.items ?? [];
            const lines = [];
            lines.push(`MetaMorpho vaults — ${network}${assetSym ? ` (filter: ${assetSym})` : ''} (top ${items.length})`);
            lines.push('');
            if (items.length === 0)
                lines.push('(no vaults match)');
            for (const v of items) {
                const curators = v.state?.curators?.map((c) => c.name).join(', ') || 'unknown';
        ...
        userByAddress(address: $address, chainId: $chainId) {
          ...
          vaultPositions {
            ...
            let data;
            try {
                data = await morphoGql(query, { address, chainId });
            }
            catch (e) {
            ...
            const active = mps.filter((p) => Number(p.state?.supplyAssetsUsd ?? 0) + Number(p.state?.borrowAssetsUsd ?? 0) + Number(p.state?.collateralUsd ?? 0) > 0);
            ...
            const activeVaults = vps.filter((p) => Number(p.state?.assetsUsd ?? 0) > 0);
            lines.push('');
