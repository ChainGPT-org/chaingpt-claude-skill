...
import { isTronAddress, tronToEvmAddress, tronToHex, base58ToHex21, } from '../lib/tron-address.js';
import { isTronMainnet, getAccount, getAccountResource, getTransactionInfoById, } from '../lib/tron.js';
import { readTrc20Balance, readTrc20Decimals, readTrc20Symbol, buildTrxTransfer, buildTrc20Transfer, } from '../lib/tron-sign.js';
import { TRON_TOKENS, resolveTronToken, assertNotPoisoned, TRX_DECIMALS, SUN_PER_TRX, TRON_DEFI, DEFAULT_FEE_LIMIT_SUN, } from '../lib/tron-tokens.js';
import { sunswapAmountsOut, justlendAccountLiquidity, justlendMarketBalances, buildJustlendTx, } from '../lib/tron-defi.js';
...
// ── unit helpers ─────────────────────────────────────────────────────
export function parseUnits(human, decimals) {
...
const fmtTrx = (sun) => `${formatUnits(sun, TRX_DECIMALS)} TRX`;
const net = (a) => String(a.network ?? 'mainnet');
function text(s) {
...
function requireAddress(addr, label = 'address') {
    const s = String(addr ?? '');
    if (!isTronAddress(s))
        throw new Error(`${label} is not a valid Tron base58 address: "${s}"`);
    assertNotPoisoned(s);
    return s;
}
function mainnetGate(network, a) {
...
export const tronTools = [
    {
        ...
        inputSchema: { type: 'object', properties: { address: { type: 'string', description: 'Tron base58 address.' } }, required: ['address'] },
    },
    {
    ...
    },
    {
        ...
        inputSchema: {
            type: 'object',
            properties: { address: { type: 'string' }, network: { type: 'string', enum: NETWORK_ENUM, default: 'mainnet' } },
            required: ['address'],
        },
    },
    {
        ...
        inputSchema: {
            type: 'object',
            properties: { txid: { type: 'string' }, network: { type: 'string', enum: NETWORK_ENUM, default: 'mainnet' } },
            required: ['txid'],
        },
    },
    {
...
async function resolveToken(network, tokenArg) {
    const curated = resolveTronToken(tokenArg);
    if (curated)
        return { address: curated.address, symbol: curated.symbol, decimals: curated.decimals };
...
export async function handleTronTool(name, args) {
    const a = args ?? {};
    try {
        switch (name) {
            case 'chaingpt_tron_validate_address': {
                ...
                if (!isTronAddress(s))
                    return text(`✗ "${s}" is not a valid Tron base58 address (T + 33 base58 chars + checksum).`);
            ...
            case 'chaingpt_tron_balances': {
                ...
                const requested = Array.isArray(a.tokens) && a.tokens.length ? a.tokens : ['USDT'];
                ...
                if (acct.balance === undefined)
                    lines.push('(account not yet activated — fund it with ~1.1 TRX to activate)');
            ...
            case 'chaingpt_tron_tx_info': {
                ...
                if (!info || Object.keys(info).length === 0)
                    return text(`No info found for tx ${a.txid} on ${network} (unconfirmed or wrong network?).`);
                return text([
            ...
            case 'chaingpt_tron_research_token': {
                ...
                const pairs = await httpJson(`https://api.dexscreener.com/token-pairs/v1/tron/${address}`);
                if (!Array.isArray(pairs) || pairs.length === 0)
                    return text(`No DexScreener pairs found for ${symbol} (${address}) on Tron.`);
            ...
            case 'chaingpt_tron_risk_token': {
                const { address, symbol } = await resolveToken('mainnet', String(a.token ?? ''));
                const r = await httpJson(`https://api.gopluslabs.io/api/v1/token_security/tron?contract_addresses=${address}`);
                const sec = r.result?.[address] ?? r.result?.[address.toLowerCase()] ?? Object.values(r.result ?? {})[0];
                if (!sec)
                    return text(`GoPlus returned no security data for ${symbol} (${address}). (code ${r.code}: ${r.message})`);
                const flag = (v) => (v === '1' ? '⚠️ YES' : v === '0' ? 'no' : String(v ?? '?'));
                return text([
            ...
            case 'chaingpt_tron_dex_sunswap_quote': {
                ...
                if (path.length < 2)
                    return text('tokenIn and tokenOut resolve to the same contract.');
            ...
            case 'chaingpt_tron_lend_justlend_account': {
                ...
                if (liq.error !== 0n)
                    lines.push(`⚠️ comptroller error code ${liq.error}`);
            ...
            case 'chaingpt_tron_lend_justlend_build_tx': {
                ...
                if (gate)
                    return text(gate);
                const marketSym = String(a.market ?? '').toUpperCase();
