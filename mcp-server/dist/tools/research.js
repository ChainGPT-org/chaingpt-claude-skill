...
export const researchTools = [
    {
        ...
        description: 'Look up live market data for a token by contract address or symbol search. Returns price, 24h volume, ' +
        ...
        inputSchema: {
            ...
            properties: {
                query: {
                    type: 'string',
                    description: 'Either a contract address (0x… on EVM, base58 on Solana) or a token symbol like "WIF" or "CGPT". ' +
    ...
    },
    {
        ...
        description: 'List trading pairs (pools) for a token contract address on a given chain. Returns up to 10 pairs sorted ' +
    ...
    },
    {
        ...
        description: 'Get the currently trending / boosted tokens on DexScreener. Useful for surfacing what is moving across ' +
...
function formatPair(p, idx) {
    const lines = [];
    const prefix = idx !== undefined ? `${idx + 1}. ` : '';
    lines.push(`${prefix}${p.baseToken.symbol}/${p.quoteToken.symbol} on ${p.chainId} via ${p.dexId}`);
    if (p.priceUsd)
        lines.push(`   Price (USD):    $${Number(p.priceUsd).toPrecision(6)}`);
    ...
    if (p.marketCap !== undefined)
        lines.push(`   Market cap:     $${formatLarge(p.marketCap)}`);
    if (p.fdv !== undefined)
        lines.push(`   FDV:            $${formatLarge(p.fdv)}`);
    lines.push(`   Pair address:   ${p.pairAddress}`);
    if (p.url)
        lines.push(`   DexScreener:    ${p.url}`);
    return lines;
}
...
export async function handleResearchTool(name, args) {
    if (!args) {
    ...
    try {
        ...
        if (name === 'chaingpt_research_trending') {
            ...
            if (chain) {
                filtered = boosted.filter((b) => b.chainId === chain.dexscreener);
            }
            ...
            for (const b of filtered) {
                if (!b?.chainId || !b?.tokenAddress)
                    continue;
                const list = byChain.get(b.chainId) ?? [];
                list.push(b.tokenAddress);
                byChain.set(b.chainId, list);
            }
            await Promise.all([...byChain.entries()].map(async ([chainId, addrs]) => {
                try {
                    const pairs = await httpJson(`https://api.dexscreener.com/tokens/v1/${chainId}/${addrs.join(',')}`);
