...
import { buildSession, getPermissionId, encodeEnableSessions, encodeRemoveSession, appendSessionRecord, readSessionRecords, SMART_SESSIONS_ADDRESS, ERC20_SPENDING_LIMIT_POLICY, SMART_SESSION_READ_ABI, } from '../lib/smart-sessions.js';
import { encodeSingleExecute, encodeInstallModule, encodeIsModuleInstalled, readAccountId, classifyAccountId, } from '../lib/erc7579.js';
 ...
 */
function chainRpcs(slug) {
    const c = resolveChainWithCustom(slug);
    if (!c?.chainId)
        throw new Error(`chain "${slug}" is not a known EVM chain.`);
    const rpcs = rpcEndpointsWithCustom(slug);
    if (!rpcs.length)
        throw new Error(`no RPC endpoints configured for ${slug}.`);
    return { chainId: c.chainId, rpcs };
}
export const aaSessionTools = [
    {
        name: 'chaingpt_aa_session_build_grant',
        description: "Build the UNSIGNED payload that grants the agent's wallet a scoped ON-CHAIN session on the user's " +
    ...
    },
    {
        ...
        description: 'Build the UNSIGNED payload that revokes an on-chain session (removeSession(permissionId)) — the ' +
    ...
    },
    {
        ...
        description: "Read a session's ON-CHAIN state: account kind, module installed?, permission enabled?, and the " +
...
export async function handleAaSessionTool(name, args) {
    args = args ?? {};
    try {
        if (name === 'chaingpt_aa_session_build_grant') {
            const { chainId, rpcs } = chainRpcs(String(args.chain));
            const account = String(args.account);
            const agentAddress = (args.agentAddress ? String(args.agentAddress) : readKeystoreFile()?.address);
            if (!agentAddress) {
            ...
            const tokenCaps = args.tokenCaps.map((t) => ({
                token: t.token,
                cap: BigInt(t.cap),
            }));
            const caps = {
                ...
                salt: args.salt ? String(args.salt) : undefined,
            };
            ...
            let installed = args.moduleInstalled !== undefined ? Boolean(args.moduleInstalled) : null;
            if (installed === null) {
                try {
                    const raw = await jsonRpcFallback(rpcs, 'eth_call', [
                        { to: account, data: encodeIsModuleInstalled(SMART_SESSIONS_ADDRESS) },
                        'latest',
                    ]);
                    installed = decodeAbiParameters([{ type: 'bool' }], raw)[0];
                }
                catch {
                    installed = null;
                }
        ...
        if (name === 'chaingpt_aa_session_status') {
            ...
            let permissionId = args.permissionId ? String(args.permissionId) : undefined;
            ...
            const lines = [`=== Session status — ${chainSlugForRpc} ===`, ``];
            try {
