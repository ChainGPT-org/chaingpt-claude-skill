...
export const intelTools = [
    {
        name: 'chaingpt_intel_token',
        description: 'AI-enriched token research. Composes: live market data (DexScreener) + security flags (GoPlus) + ' +
    ...
    },
    {
        ...
        description: 'AI-enriched wallet research. Pulls portfolio holdings, risk-rates the top tokens via GoPlus, and ' +
...
async function fetchSignal(symbol) {
    ...
    if (!key)
        return null;
    try {
        const res = await httpJson(`${CHAINGPT_API_BASE}/ai-signal/details?symbol=${encodeURIComponent(symbol)}`, {
...
export async function handleIntelTool(name, args) {
    if (!args) {
    ...
    try {
        if (name === 'chaingpt_intel_token') {
            ...
            try {
                ...
                pairs = dsRes.pairs ?? [];
                if (chain)
                    pairs = pairs.filter((p) => p.chainId === chain.dexscreener);
            ...
            if (contract && resolvedChain?.goplus && resolvedChain.chainId !== null) {
                try {
                    const goRes = await httpJson(`${GOPLUS_BASE}/token_security/${resolvedChain.goplus}?contract_addresses=${contract}`);
            ...
            let newsArticles = [];
            try {
                const newsRes = await newsClient().getNews({
                    searchQuery: symbol,
                    limit: newsLimit,
                    offset: 0,
                    sortBy: 'createdAt',
            ...
            if (top) {
                if (top.priceUsd)
                    lines.push(`  Price (USD):       $${Number(top.priceUsd).toPrecision(6)}`);
                if (top.priceChange?.h24 !== undefined)
                    lines.push(`  24h change:        ${top.priceChange.h24 >= 0 ? '+' : ''}${top.priceChange.h24.toFixed(2)}%`);
                if (top.volume?.h24)
                    lines.push(`  24h volume:        $${formatLarge(top.volume.h24)}`);
                if (top.liquidity?.usd)
                    lines.push(`  Liquidity:         $${formatLarge(top.liquidity.usd)}`);
                if (top.marketCap)
                    lines.push(`  Market cap:        $${formatLarge(top.marketCap)}`);
                if (top.fdv)
                    lines.push(`  FDV:               $${formatLarge(top.fdv)}`);
            ...
            if (!contract) {
                ...
                if (riskMeta.holder_count)
                    lines.push(`  Holders:           ${riskMeta.holder_count}`);
            ...
            if (!signal) {
                ...
                if (signal.bullishness !== undefined)
                    lines.push(`  Bullishness:       ${signal.bullishness}`);
                if (signal.tag)
                    lines.push(`  Tag/narrative:     ${signal.tag}`);
                if (!signal.bullishness && !signal.tag)
                    lines.push('  (signal returned but empty)');
            }
            lines.push('');
        ...
        if (name === 'chaingpt_intel_wallet') {
            ...
            const chainResults = await Promise.allSettled(validChains.map(async (slug) => httpJson(`https://deep-index.moralis.io/api/v2.2/wallets/${address}/tokens?chain=${moralisMap[slug]}`, { headers: { 'X-API-Key': moralisKey } }).then((res) => ({ slug, tokens: res.result ?? [] }))));
            const allHoldings = [];
            for (const r of chainResults) {
                if (r.status !== 'fulfilled')
                    continue;
                for (const t of r.value.tokens) {
                    if ((t.usd_value ?? 0) >= minUsd)
                        allHoldings.push({ chainSlug: r.value.slug, token: t });
                }
            }
            ...
            for (const { chainSlug, token } of top) {
                ...
                if (!c?.goplus || token.native_token) {
                    lines.push(`  ${safeSymbol.padEnd(10)} $${(token.usd_value ?? 0).toFixed(2).padStart(10)}  [${chainSlug}]  (native or risk-skip)`);
                    continue;
                }
                let verdict = '?';
                try {
                    const goRes = await httpJson(`${GOPLUS_BASE}/token_security/${c.goplus}?contract_addresses=${token.token_address}`);
                    ...
                    if (!row) {
                        ...
                        const flagged = [];
                        if (row.is_honeypot === '1')
                            flagged.push('honeypot');
                        if (row.cannot_sell_all === '1')
                            flagged.push('cannot-sell-all');
                        if (row.is_mintable === '1')
                            flagged.push('mintable');
                        if (row.hidden_owner === '1')
                            flagged.push('hidden-owner');
                ...
                lines.push(`  ${safeSymbol.padEnd(10)} $${(token.usd_value ?? 0).toFixed(2).padStart(10)}  [${chainSlug}]  ${verdict}`);
            }
            lines.push('');
