export const utilTools = [
    {
        ...
        description: 'Estimate the credit cost for a ChainGPT API operation before executing it. Returns credit cost and USD equivalent. 1 credit = $0.01 USD.',
        inputSchema: {
            type: 'object',
            properties: {
                product: {
    ...
    },
    {
        ...
        description: 'Get instructions for checking your ChainGPT API credit balance and topping up. The balance is not available via API — this returns the dashboard URL.',
        inputSchema: {
            type: 'object',
            properties: {},
            required: [],
...
function estimateCredits(product, options = {}) {
    const breakdown = [];
    let credits = 0;
    switch (product) {
        case 'chat': {
        ...
        case 'nft': {
            const model = options.model || 'velogen';
            const enhance = options.enhance || 'original';
        ...
        case 'news': {
            const limit = options.newsLimit || 10;
...
export async function handleUtilTool(name, args) {
    if (!args)
        args = {};
    try {
