...
export const bridgeTools = [
    {
        ...
        description: 'Get a cross-chain bridge quote via Across Protocol v3. Returns fees (capital + gas + LP), estimated ' +
    ...
    },
    {
        ...
        description: 'Build an UNSIGNED Across V3 depositV3 transaction for the user\'s wallet to sign + broadcast on the ' +
    ...
    },
    {
        ...
        description: 'Check the status of an Across bridge deposit by transaction hash. Returns whether the deposit was ' +
...
export async function handleBridgeTool(name, args) {
    if (!args)
        ...
        if (name === 'chaingpt_bridge_quote' || name === 'chaingpt_bridge_build_deposit_tx') {
            ...
            if (name === 'chaingpt_bridge_build_deposit_tx' && args.acknowledgeMainnet !== true) {
                return {
                    content: [{
                            type: 'text',
                            text: `⚠ Mainnet bridge refused. To bridge ${args.amount} on ${originChain.name} → ${destChain.name}, ` +
            ...
            const url = `${ACROSS_BASE}/suggested-fees?` +
                new URLSearchParams({
                    inputToken,
                    outputToken,
            ...
            const quote = await httpJson(url);
            ...
            if (outputAmount <= 0n) {
                return {
                    content: [{
                            type: 'text',
                            text: `Across bridge refused: relay fee (${relayFeeTotal}) + LP fee (${lpFeeTotal}) ≥ input ` +
            ...
            const data = encodeFunctionData({
                ...
                args: [
                    ...
                    (quote.exclusiveRelayer ?? '0x0000000000000000000000000000000000000000'),
            ...
            const isNative = inputToken.toLowerCase() === '0x0000000000000000000000000000000000000000' ||
        ...
        if (name === 'chaingpt_bridge_status') {
            ...
            try {
                ...
                return {
                    content: [{
                            type: 'text',
                            text: `Across status check failed: ${msg}\n` +
