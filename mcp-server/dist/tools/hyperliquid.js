...
async function info(body) {
    return httpJson(HL_INFO, { method: 'POST', body });
}
export const hyperliquidTools = [
    {
        name: 'chaingpt_hl_markets',
        description: 'List Hyperliquid perpetual + spot markets (assets, max leverage, decimals). Read-only. 0 ChainGPT credits.',
        inputSchema: {
            type: 'object',
            properties: {
                type: {
    ...
    },
    {
        ...
        description: 'Get live mid prices for all Hyperliquid assets in one call. Useful for quick portfolio mark-to-market. ' +
    ...
    },
    {
        ...
        description: 'Get the L2 orderbook for a Hyperliquid asset (best bids + asks, by price level). Read-only. ' +
            '0 ChainGPT credits.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Get the full Hyperliquid account state for a wallet: USDC balance, margin used, account value, ' +
    ...
    },
    {
        ...
        description: 'Recent fill history for a Hyperliquid account. Returns side, coin, price, size, fee, PnL per fill. ' +
            'Read-only. 0 ChainGPT credits.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Funding-rate history for a Hyperliquid perp asset. Returns the last 24h of hourly funding rates. ' +
            'Read-only. 0 ChainGPT credits.',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    // ─── Signed-action payload builders ─────────────────────────────────
    {
        ...
        description: 'Build the action + EIP-712 typed-data payload for a Hyperliquid limit order. The plugin does NOT ' +
    ...
    },
    {
        ...
        description: 'Build the action + EIP-712 typed-data payload for a Hyperliquid order cancel. Cancels do not require ' +
    ...
    },
    {
        ...
        description: 'POST a signed Hyperliquid action to /exchange. Takes the action object + nonce returned by a ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                signature: {
                    description: 'The signature from the user\'s wallet. Either an object {r, s, v} or a 0x-prefixed hex string.',
                },
...
export async function handleHyperliquidTool(name, args) {
    if (!args) {
    ...
    try {
        ...
        if (name === 'chaingpt_hl_mids') {
            ...
            const filter = Array.isArray(args.filter)
                ? new Set(args.filter.filter((x) => typeof x === 'string').map((s) => s.toUpperCase()))
                : null;
            ...
            for (const [coin, price] of entries)
                lines.push(`  ${coin.padEnd(12)} ${price}`);
            return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
        if (name === 'chaingpt_hl_orderbook') {
            const coin = String(args.coin || '').toUpperCase();
            if (!coin)
                return { content: [{ type: 'text', text: 'coin is required.' }] };
        ...
        if (name === 'chaingpt_hl_account') {
            ...
            if (!user)
                return { content: [{ type: 'text', text: 'user is required.' }] };
            const state = await info({ type: 'clearinghouseState', user });
            ...
            if (positions.length === 0) {
                ...
                for (const p of positions) {
                    ...
                    lines.push(`  ${side} ${pos.coin?.padEnd(8) ?? '?'}  size=${formatNum(sz, 4)}  entry=$${formatNum(pos.entryPx)}  ` +
        ...
        if (name === 'chaingpt_hl_fills') {
            ...
            for (const f of slice) {
                ...
                lines.push(`  ${ts}  ${side} ${(f.coin ?? '?').padEnd(8)} ${formatNum(f.sz, 4).padStart(12)} @ $${formatNum(f.px)}  ` +
        ...
        if (name === 'chaingpt_hl_funding') {
            ...
            const history = await info({ type: 'fundingHistory', coin, startTime });
            const slice = Array.isArray(history) ? history : [];
            if (slice.length === 0) {
                return { content: [{ type: 'text', text: `No funding history for ${coin} in last ${hours}h.` }] };
            }
            const lines = [`Hyperliquid funding history — ${coin} (last ${hours}h):`, ''];
        ...
        if (name === 'chaingpt_hl_place_order_payload') {
            if (args.acknowledgeMainnet !== true) {
                return {
                    content: [{
                            type: 'text',
                            text: `⚠ Hyperliquid mainnet order refused. Pass acknowledgeMainnet: true to receive the signing ` +
        ...
        if (name === 'chaingpt_hl_submit_signed_action') {
            ...
            const res = await httpJson(HL_EXCHANGE, { method: 'POST', body });
