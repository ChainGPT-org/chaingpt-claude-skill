...
export const strategyTools = [
    {
        ...
        description: 'Build a dollar-cost-average plan for one token. Given a total budget, number of buys, and cadence, ' +
    ...
    },
    {
        ...
        description: 'Build a grid-trading ladder of limit orders between a lower and upper bound. Returns evenly-spaced ' +
    ...
    },
    {
        ...
        description: 'Suggest a Hyperliquid funding-rate arbitrage position. Given the current funding rate for a coin, ' +
    ...
    },
    {
        ...
        description: 'Build a copy-trading plan: given a target wallet, fetch its recent DEX swaps and propose mirrored ' +
    ...
    },
    {
        ...
        description: 'Backtest a DCA strategy against historical price data from CoinGecko. Replays buys at fixed intervals ' +
    ...
    },
    {
        ...
        description: 'Backtest a grid-trading strategy against historical prices. Replays a fixed buy/sell ladder between ' +
...
export async function handleStrategyTool(name, args) {
    ...
    try {
        ...
        if (name === 'chaingpt_strategy_grid_plan') {
            ...
            buyLevels.forEach((l, i) => lines.push(`  ${(i + 1).toString().padStart(2)}. buy ${formatUsd(l.usd)} at ${l.price.toFixed(6)}`));
            lines.push('');
            lines.push('SELL ladder (limit orders above mid):');
            sellLevels.forEach((l, i) => lines.push(`  ${(i + 1).toString().padStart(2)}. sell ${formatUsd(l.usd)} at ${l.price.toFixed(6)}`));
            lines.push('');
        ...
        if (name === 'chaingpt_strategy_funding_arb_plan') {
            ...
            const history = await httpJson('https://api.hyperliquid.xyz/info', {
                method: 'POST',
            ...
            if (absAnnualized < minAnnualizedPct) {
                return {
                    content: [{
                            type: 'text',
                            text: `${coin} funding is ${(latest * 100).toFixed(4)}% / hr (annualized ${annualizedPct.toFixed(2)}%) — below the ${minAnnualizedPct}% threshold. ` +
                                `Skip this arb.`,
                        }],
                };
            }
        ...
        if (name === 'chaingpt_backtest_grid') {
            ...
            const openOrders = buyLevels.map((p) => ({ side: 'buy', price: p, size: perBuyQuote / p }));
