...
import { hexToNumber, hexWeiToGwei, httpJson, jsonRpcFallback } from '../lib/http.js';
...
export const onchainTools = [
    {
        ...
        description: 'Look up a transaction by hash. Returns status (success/failed/pending), from/to, value, gas used, ' +
    ...
    },
    {
        ...
        description: 'Get the recent transaction history for an address on an EVM chain. Returns up to 25 most-recent ' +
    ...
    },
    {
        ...
        description: 'Get current gas prices for an EVM chain. Returns safe / standard / fast gwei estimates plus the ' +
    ...
    },
    {
        ...
        description: 'Get a block by number, or the latest block. Returns timestamp, miner/proposer, tx count, gas used, ' +
...
export async function handleOnchainTool(name, args) {
    if (!args) {
    ...
    try {
        if (name === 'chaingpt_onchain_tx') {
            ...
            if (!hash)
                return { content: [{ type: 'text', text: 'Error: hash is required.' }] };
            const chain = resolveChain(args.chain ?? 'ethereum');
            ...
            const txUrl = `${ETHERSCAN_V2}?chainid=${chain.chainId}&module=proxy&action=eth_getTransactionByHash` +
                `&txhash=${hash}&apikey=${etherscanKey() ?? 'YourApiKeyToken'}`;
            const txRes = await httpJson(txUrl);
            const txKeyHint = detectMissingKey(txRes);
            if (txKeyHint)
                return { content: [{ type: 'text', text: txKeyHint }] };
            const tx = txRes.result;
            if (!tx) {
                return { content: [{ type: 'text', text: `No transaction found for ${hash} on ${chain.name}.` }] };
            }
            const receiptUrl = `${ETHERSCAN_V2}?chainid=${chain.chainId}&module=proxy&action=eth_getTransactionReceipt` +
                `&txhash=${hash}&apikey=${etherscanKey() ?? 'YourApiKeyToken'}`;
            const receiptRes = await httpJson(receiptUrl);
            const receiptKeyHint = detectMissingKey(receiptRes);
            if (receiptKeyHint)
                return { content: [{ type: 'text', text: receiptKeyHint }] };
            ...
            if (receipt) {
                ...
                if (Array.isArray(receipt.logs))
                    lines.push(`Log entries:     ${receipt.logs.length}`);
            }
            else {
        ...
        if (name === 'chaingpt_onchain_address') {
            ...
            const url = `${ETHERSCAN_V2}?chainid=${chain.chainId}&module=account&action=txlist&address=${address}` +
                `&startblock=0&endblock=99999999&page=1&offset=${limit}&sort=desc&apikey=${etherscanKey()}`;
            const res = await httpJson(url);
            const keyHint = detectMissingKey(res);
            if (keyHint)
                return { content: [{ type: 'text', text: keyHint }] };
            ...
            for (const tx of txs) {
                ...
                lines.push(`${status} ${formatTimestamp(tx.timeStamp).slice(0, 19).replace('T', ' ')}  ${dir} ${counter}  ${formatWei(tx.value)} ${chain.native}  [${method}]`);
            }
            return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
        if (name === 'chaingpt_onchain_gas') {
            ...
            try {
                ...
                if (res.status === '1' && res.result) {
                    ...
                    const lines = [
                        ...
                        r.gasUsedRatio
                            ? `Utilization: ${((String(r.gasUsedRatio).split(',').map(Number).filter(Number.isFinite).reduce((a, b) => a + b, 0) /
                                Math.max(1, String(r.gasUsedRatio).split(',').map(Number).filter(Number.isFinite).length)) * 100).toFixed(0)}% (avg of last ${String(r.gasUsedRatio).split(',').length} blocks)`
                            : '',
                    ].filter(Boolean);
                    return { content: [{ type: 'text', text: lines.join('\n') }] };
                }
            ...
            const gasPriceHex = await jsonRpcFallback(rpcs, 'eth_gasPrice', []);
            const lines = [
        ...
        if (name === 'chaingpt_onchain_block') {
            ...
            const block = await jsonRpcFallback(rpcs, 'eth_getBlockByNumber', [blockTag, false]);
            if (!block)
                return { content: [{ type: 'text', text: `Block not found: ${raw}` }] };
            const lines = [
