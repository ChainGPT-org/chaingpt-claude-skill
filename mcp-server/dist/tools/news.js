...
export const newsTools = [
    {
        ...
        description: 'Fetch AI-curated crypto and blockchain news articles. Supports filtering by category, blockchain, token, search text, and date. Costs 1 credit per 10 records returned (rounded up). Use chaingpt_news_categories to look up valid IDs.',
        inputSchema: {
            type: 'object',
            properties: {
                categories: {
                    type: 'array',
                    items: { type: 'number' },
                    description: 'Category IDs to filter by. Common: 5=DeFi, 8=NFT, 64=Cryptocurrency, 78=Exchange, 2=Gaming, 3=DAO, 4=DApps, 6=Lending, 7=Metaverse, 9=Stablecoins, 66=Smart Contracts, 74=Web3. Also: 65=Decentralized, 67=Distributed Ledger, 68=Cryptography, 69=Digital Assets, 70=Tokenization, 71=Consensus, 72=ICO, 73=Wallets, 75=Interoperability, 76=Mining, 77=Cross-Chain',
                },
                blockchains: {
                    type: 'array',
                    items: { type: 'number' },
                    description: 'Blockchain sub-category IDs. Common: 11=Bitcoin, 12=BNB, 15=Ethereum, 20=Polygon, 22=Solana, 28=Arbitrum, 31=Avalanche, 32=Base, 45=Optimism',
                },
                tokens: {
                    type: 'array',
                    items: { type: 'number' },
                    description: 'Token IDs. Common: 79=BTC, 80=ETH, 81=USDT, 82=BNB, 83=XRP, 85=SOL, 87=DOGE, 96=LINK, 108=CGPT',
                },
                search: {
                    type: 'string',
    ...
    },
    {
        ...
        description: 'Get the complete reference list of all category IDs, blockchain IDs, and token IDs for use with chaingpt_news_fetch. Free (0 credits).',
        inputSchema: {
            type: 'object',
            properties: {},
            required: [],
...
export async function handleNewsTool(name, args) {
    if (!args)
        args = {};
    try {
        ...
        if (name === 'chaingpt_news_fetch') {
            ...
            if (args.fetchAfter)
                params.fetchAfter = args.fetchAfter;
            const response = await getClient().getNews(params);
            ...
            if (Array.isArray(data) && data.length > 0) {
                const articles = data.map((article, i) => {
                    ...
                    if (article.subCategory?.name)
                        parts.push(`   Blockchain: ${article.subCategory.name}`);
                    if (article.token?.symbol)
                        parts.push(`   Token: ${article.token.symbol}`);
