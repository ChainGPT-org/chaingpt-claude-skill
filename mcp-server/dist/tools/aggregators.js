...
export const aggregatorTools = [
    // ─── 1inch v6 ─────────────────────────────────────────────────────
    {
        ...
        description: 'Get a swap quote from 1inch v6. Generally better routing than OpenOcean on Ethereum + L2s for large ' +
    ...
    },
    {
        ...
        description: 'Build an UNSIGNED 1inch v6 swap transaction. Requires ONEINCH_API_KEY + acknowledgeMainnet=true. ' +
    ...
    // ─── CoW Protocol ─────────────────────────────────────────────────
    {
        ...
        description: 'Create a CoW Protocol swap order. CoW uses an intent model: instead of returning a tx, this returns ' +
    ...
    },
    {
        ...
        description: 'Submit a signed CoW Protocol order. The user signs the order returned by chaingpt_dex_cow_create_order ' +
...
function formatAmount(raw, decimals, decimalsToShow = 4) {
    try {
...
export async function handleAggregatorTool(name, args) {
    if (!args)
        ...
        if (name === 'chaingpt_dex_1inch_quote' || name === 'chaingpt_dex_1inch_swap_tx') {
            ...
            if (!key) {
                return {
                    content: [{
                            type: 'text',
                            text: `1inch v6 requires ONEINCH_API_KEY. Get a free key at https://1inch.dev (Developer Portal → ` +
            ...
            if (name === 'chaingpt_dex_1inch_swap_tx' && args.acknowledgeMainnet !== true) {
                return {
                    content: [{
                            type: 'text',
                            text: `⚠ Mainnet swap refused. Pass acknowledgeMainnet: true to receive the 1inch unsigned tx. ` +
            ...
            const res = await httpJson(url, { headers: oneinchHeaders(key) });
            ...
            if (res.gas)
                lines.push(`Est. gas:        ${res.gas}`);
            if (res.protocols && Array.isArray(res.protocols)) {
                const protoNames = res.protocols.flat(2).map((p) => p.name).filter(Boolean);
        ...
        if (name === 'chaingpt_dex_cow_create_order') {
            if (args.acknowledgeMainnet !== true) {
                return {
                    content: [{
                            type: 'text',
                            text: `⚠ CoW order refused without acknowledgeMainnet. CoW orders are real-money trades — once ` +
            ...
            const buyToken = String(args.buyToken).toLowerCase();
            ...
            const quoteRes = await httpJson(`${COW_API_BASE}/${cowNet}/api/v1/quote`, {
                method: 'POST',
                body: {
                    sellToken,
                    buyToken,
            ...
            const buyAmountMin = (BigInt(q.buyAmount) * BigInt(10_000 - slippageBps)) / 10000n;
            const order = {
