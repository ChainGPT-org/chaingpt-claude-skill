...
export const riskTools = [
    {
        ...
        description: 'Run a token-security check via GoPlus Labs. Flags honeypot, mintable, blacklist functions, ownership ' +
    ...
    },
    {
        ...
        description: 'Simulate a buy + sell of a token via Honeypot.is to detect honeypots, hidden taxes, and gas-bomb ' +
    ...
    },
    {
        ...
        description: 'Check whether a wallet or contract address is flagged for malicious behavior via GoPlus: sanctions, ' +
    ...
    },
    {
        ...
        description: 'Fetch the verified source code, ABI, and compiler settings for a contract from Etherscan v2 ' +
...
export async function handleRiskTool(name, args) {
    if (!args) {
    ...
    try {
        if (name === 'chaingpt_risk_token') {
            ...
            if (row.holder_count)
                lines.push(`Holders:         ${row.holder_count}`);
        ...
        if (name === 'chaingpt_risk_honeypot') {
            ...
            if (tax.buyTax !== undefined)
                lines.push(`Buy tax:         ${Number(tax.buyTax).toFixed(2)}%`);
            if (tax.sellTax !== undefined)
                lines.push(`Sell tax:        ${Number(tax.sellTax).toFixed(2)}%`);
            if (tax.transferTax !== undefined)
                lines.push(`Transfer tax:    ${Number(tax.transferTax).toFixed(2)}%`);
            if (res?.summary?.risk)
                lines.push(`Overall risk:    ${res.summary.risk}`);
        ...
        if (name === 'chaingpt_risk_contract_source') {
            ...
            const url = `${ETHERSCAN_V2_BASE}?chainid=${chain.chainId}&module=contract&action=getsourcecode` +
            ...
            if (!row || !row.SourceCode) {
                return {
                    content: [{
                            type: 'text',
                            text: `No verified source code for ${address} on ${chain.name}.\n` +
            ...
            if (source.startsWith('{{') && source.endsWith('}}')) {
                try {
                    ...
                    if (parsed.sources && typeof parsed.sources === 'object') {
                        ...
                        const firstFile = Object.entries(parsed.sources)[0];
                        source = `// ${firstFile[0]}\n${firstFile[1].content ?? ''}`;
                    }
                }
                catch {
