...
export const auditTools = [
    {
        ...
        description: 'Audit a Solidity smart contract for security vulnerabilities, gas inefficiencies, access control issues, reentrancy, integer overflow, and more. Returns a scored audit report (0-100%) with categorized findings by severity (Critical/High/Medium/Low/Informational) and remediation suggestions. Costs 1 credit (2 with chat history). Supports follow-up questions about the same audit via sessionId.',
        inputSchema: {
            type: 'object',
            properties: {
                sourceCode: {
                    type: 'string',
                    description: 'The Solidity smart contract source code to audit. Include pragma and all imports.',
                },
                auditInstructions: {
                    type: 'string',
                    description: 'Optional specific audit instructions (e.g. "focus on reentrancy and access control")',
                },
                followUpQuestion: {
                    type: 'string',
                    description: 'A follow-up question about a previous audit (requires sessionId). When provided, sourceCode is not needed.',
                },
                sessionId: {
                    type: 'string',
                    description: 'Session ID for multi-turn audit conversations. Required for follow-up questions.',
                },
            },
            required: [],
        },
    },
    {
        ...
        description: 'Retrieve audit conversation history for a given session. Requires a session ID from previous audit calls.',
        inputSchema: {
            type: 'object',
            properties: {
                sessionId: {
...
export async function handleAuditTool(name, args) {
    if (!args) {
