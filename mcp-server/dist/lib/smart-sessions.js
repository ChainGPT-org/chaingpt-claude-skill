...
export const ERC20_SPENDING_LIMIT_POLICY = '0x000000000033212e272655d8a22402db819477a6';
export const TIME_FRAME_POLICY = '0x0000000000D30f611fA3bf652ac6879428586930';
export const USAGE_LIMIT_POLICY = '0x00000000001d4479FA2A947026204d0283ceDe4B';
export const VALUE_LIMIT_POLICY = '0x000000000021dC45451291BCDfc9f0B46d6f0278';
// 65-byte mock ECDSA signature for bundler gas estimation (r=1, s=1, v=27).
export const MOCK_ECDSA_SIG = `0x${'00'.repeat(31)}01${'00'.repeat(31)}011b`;
...
function timeFramePolicy(validUntil, validAfter = 0) {
...
function usageLimitPolicy(maxUses) {
    return { policy: USAGE_LIMIT_POLICY, initData: encodePacked(['uint128'], [maxUses]) };
}
function valueLimitPolicy(cap) {
    return { policy: VALUE_LIMIT_POLICY, initData: encodeAbiParameters([{ type: 'uint256' }], [cap]) };
}
function spendingLimitPolicy(tokens, caps) {
...
export function buildSession(caps) {
    ...
    for (const t of caps.tokenCaps) {
        if (t.cap <= 0n)
            throw new Error(`Token cap for ${t.token} must be > 0 (the on-chain policy reverts on zero limits).`);
    }
    const timePolicies = [timeFramePolicy(caps.validUntil, caps.validAfter ?? 0)];
    if (caps.maxUses !== undefined)
        timePolicies.push(usageLimitPolicy(caps.maxUses));
    const actions = [];
    for (const t of caps.tokenCaps) {
        actions.push({
    ...
    for (const extra of caps.targets ?? []) {
        const pols = [...timePolicies];
        if (caps.nativeValueCap !== undefined)
            pols.push(valueLimitPolicy(caps.nativeValueCap));
    ...
    return {
        ...
        sessionValidatorInitData: encodeAbiParameters([{ type: 'uint256' }, { type: 'address[]' }], [1n, [caps.agentAddress]]),
        salt: caps.salt ?? `0x${'00'.repeat(32)}`,
...
export function getPermissionId(session) {
    return keccak256(encodeAbiParameters([{ type: 'address' }, { type: 'bytes' }, { type: 'bytes32' }], [session.sessionValidator, session.sessionValidatorInitData, session.salt]));
}
...
export function encodeEnableSessions(sessions) {
    return encodeFunctionData({ abi: ENABLE_SESSIONS_ABI, functionName: 'enableSessions', args: [sessions] });
}
export function encodeRemoveSession(permissionId) {
...
export function encodeUseSignature(permissionId, sig) {
    return encodePacked(['bytes1', 'bytes32', 'bytes'], ['0x00', permissionId, sig]);
}
