...
export function saveTrackedTokens(tokens) {
    const path = tokensPath();
...
export function addTrackedToken(t) {
    if (!HEX_ADDR_RE.test(t.address))
        throw new Error(`Invalid token address: ${t.address}`);
    if (!t.symbol)
...
export function removeTrackedToken(chain, address) {
    const tokens = loadTrackedTokens();
