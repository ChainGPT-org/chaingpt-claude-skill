...
import { triggerConstantContract, triggerSmartContract, createTransaction, decodeBroadcastMessage, } from './tron.js';
...
export function encodeParams(params, values) {
    return encodeAbiParameters(params, values).slice(2);
}
// ── ABI decoding (outputs) — viem (no address-checksum concern on decode) ──
export function decodeUint(res) {
    const hex = res.startsWith('0x') ? res : `0x${res}`;
    const [v] = decodeAbiParameters([{ type: 'uint256' }], hex);
    return v;
}
export function decodeStringResult(res) {
    const hex = res.startsWith('0x') ? res : `0x${res}`;
    const [v] = decodeAbiParameters([{ type: 'string' }], hex);
    return v;
}
// ── TRC-20 reads (via triggerconstantcontract) ───────────────────────
export async function readTrc20Decimals(network, owner, token) {
    ...
    if (!res)
        throw new Error(`decimals() on ${token} returned no result`);
    return Number(decodeUint(res));
}
export async function readTrc20Symbol(network, owner, token) {
    const r = await triggerConstantContract(network, { ownerBase58: owner, contractBase58: token, functionSelector: 'symbol()' });
    const res = r.constant_result?.[0];
    if (!res)
        throw new Error(`symbol() on ${token} returned no result`);
    return decodeStringResult(res);
}
export async function readTrc20Balance(network, token, holder) {
    ...
    const res = r.constant_result?.[0];
    if (!res)
        throw new Error(`balanceOf on ${token} returned no result`);
    return decodeUint(res);
}
// ── Signing ──────────────────────────────────────────────────────────
/** The agent's base58 Tron address — derived from the same secp256k1 EVM key. */
export function deriveTronAddress(account) {
...
export function computeTxId(rawDataHex) {
...
export function verifyTxId(rawDataHex, txID) {
    if (!rawDataHex || !txID)
        return false;
 ...
 */
export async function signTxId(account, txID) {
    const clean = txID.replace(/^0x/, '').toLowerCase();
    if (!/^[0-9a-f]{64}$/.test(clean))
        throw new Error('txID must be 32-byte hex');
    if (typeof account.sign !== 'function')
        throw new Error('agent account cannot sign a raw hash (viem account.sign missing)');
    ...
    if (recid !== 0 && recid !== 1)
        throw new Error(`unexpected recovery id ${recid}`);
 ...
 */
export async function signUnsignedTx(account, tx) {
...
export function decodeRawData(rawData) {
    ...
    if (!c)
        throw new Error('raw_data has no contract');
...
export async function buildTrxTransfer(network, args) {
    if (args.amountSun <= 0n)
        throw new Error('amount must be positive');
    return createTransaction(network, args);
}
export async function buildTrc20Transfer(network, args) {
    if (args.amount <= 0n)
        throw new Error('amount must be positive');
...
export async function buildContractCall(network, args) {
 ...
 */
export async function constantPrecheck(network, args) {
