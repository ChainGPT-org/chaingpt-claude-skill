...
export function withEtherscanKey(params) {
...
export function detectMissingKey(res) {
    ...
    if (rs.toLowerCase().includes('missing/invalid api key') ||
        ...
        if (rs.toLowerCase().includes('api key')) {
            return ('Etherscan rejected the request — no valid `ETHERSCAN_API_KEY` is set. ' +
