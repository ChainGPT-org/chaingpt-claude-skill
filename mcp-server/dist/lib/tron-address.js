...
export function normalizeEvmAddress(evm) {
...
export function evmToHex21(evm) {
...
export function hex21ToBase58(hex21) {
    if (hex21.length !== 21)
        throw new Error(`Tron payload must be 21 bytes, got ${hex21.length}`);
    if (hex21[0] !== TRON_MAINNET_PREFIX)
        throw new Error(`Tron payload must start with 0x41, got 0x${hex21[0].toString(16)}`);
    return base58Encode(concatBytes(hex21, checksum4(hex21)));
}
/** EVM address (0x… 20-byte) → base58 Tron "T…" address. */
export function tronAddressFromEvm(evm) {
 ...
 */
export function base58ToHex21(addr) {
    ...
    if (decoded.length !== 25)
        throw new Error(`Tron address decodes to ${decoded.length} bytes, expected 25`);
    const payload = decoded.slice(0, 21);
    const check = decoded.slice(21);
    const expected = checksum4(payload);
    for (let i = 0; i < 4; i++) {
        if (check[i] !== expected[i])
            throw new Error(`Tron address checksum mismatch for "${addr}"`);
    }
    if (payload[0] !== TRON_MAINNET_PREFIX)
        throw new Error(`Tron address is not mainnet (prefix 0x${payload[0].toString(16)})`);
    return payload;
}
...
export function tronToEvmAddress(addr) {
 ...
 */
export function toEvmAddressParam(addr) {
    if (isTronAddress(addr))
        return tronToEvmAddress(addr);
 ...
 */
export function normalizeToBase58(addr) {
    ...
    if (/^41[0-9a-fA-F]{40}$/.test(hex))
        return hex21ToBase58(hexToBytes(hex));
    if (/^[0-9a-fA-F]{40}$/.test(hex))
        return tronAddressFromEvm(s);
