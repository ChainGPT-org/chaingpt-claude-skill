...
import { encryptSecret, decryptSecret, resolveOrProvisionPassphrase, } from './agent-keystore.js';
import { resolvePassphrase } from './agent-secret.js';
const DEFAULT_PATH = join(homedir(), '.chaingpt-mcp', 'agent-wallet', 'solana-keystore.json');
export function solanaKeystorePath() {
    return process.env.CHAINGPT_SOLANA_KEYSTORE_FILE?.trim() || DEFAULT_PATH;
}
export function isSolanaKeystoreInitialized() {
    return existsSync(solanaKeystorePath());
}
export function readSolanaKeystoreFile() {
 ...
 */
export function initSolanaKeystore() {
    const path = solanaKeystorePath();
    if (isSolanaKeystoreInitialized()) {
        throw new Error(`Solana keystore already exists at ${path}. ` +
...
export function loadSolanaKeypair() {
    ...
    if (!pass) {
        throw new Error('No keystore passphrase available; cannot decrypt the Solana keystore. ' +
    ...
    if (!file) {
        throw new Error(`Solana keystore not found at ${solanaKeystorePath()}. ` +
