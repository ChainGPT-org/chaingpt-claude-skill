 ...
 */
import { concatHex, encodeAbiParameters, encodeFunctionData, encodePacked, decodeAbiParameters } from 'viem';
 ...
 */
export const MODE_SINGLE_DEFAULT = `0x${'00'.repeat(32)}`;
/** execute(single): executionCalldata = packed(target ++ value ++ data) */
export function encodeSingleExecute(target, value, data) {
...
export function encodeInstallModule(module, initData) {
    return encodeFunctionData({
 ...
 */
export function nexusNonceKey(validator) {
    const hex = `0x${'00'.repeat(3)}00${validator.slice(2)}`; // 24 bytes = uint192
    return BigInt(hex);
}
/** Classify an ERC-7579 accountId() string (e.g. "biconomy.nexus.1.0.2"). */
export function classifyAccountId(id) {
...
export function encodeGetNonce(sender, key) {
