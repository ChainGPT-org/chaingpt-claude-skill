...
const DEFAULT_POLICY = {
    ...
    notes: 'Balanced DeFi default (killSwitch OFF). The agent may interact with major DEX aggregators ' +
...
export function policyDigest(p) {
...
export function checkPolicy(intent, policy = loadPolicy(), spend) {
    ...
    if (policy.maxTxValueWei !== undefined) {
        let max;
        try {
            max = BigInt(policy.maxTxValueWei);
        }
        catch {
            return { allowed: false, reason: `Policy maxTxValueWei is not a valid integer string.`, policyDigest: digest };
        }
        if (intent.value > max) {
    ...
    if (policy.maxTxGas !== undefined) {
        let max;
        try {
            max = BigInt(policy.maxTxGas);
        }
        catch {
            return { allowed: false, reason: `Policy maxTxGas is not a valid integer string.`, policyDigest: digest };
        }
    ...
    if (hasVelocityCap) {
        ...
        if (policy.maxDailySpendWei !== undefined) {
            let maxSpend;
            try {
                maxSpend = BigInt(policy.maxDailySpendWei);
            }
            catch {
                return { allowed: false, reason: `Policy maxDailySpendWei is not a valid integer string.`, policyDigest: digest };
            }
 ...
 */
export function checkSolanaPolicy(intent, policy = loadPolicy(), spend) {
    const digest = policyDigest(policy);
    if (policy.killSwitch) {
        return { allowed: false, reason: 'Policy kill switch is active. Edit policy.json (set killSwitch: false) to allow operations.', policyDigest: digest };
    }
    ...
    if (sol.maxTxLamports !== undefined) {
        let max;
        try {
            max = BigInt(sol.maxTxLamports);
        }
        catch {
            return { allowed: false, reason: 'Policy solana.maxTxLamports is not a valid integer string.', policyDigest: digest };
        }
    ...
    if (hasVelocityCap) {
        ...
        if (sol.maxDailySpendLamports !== undefined) {
            let maxSpend;
            try {
                maxSpend = BigInt(sol.maxDailySpendLamports);
            }
            catch {
                return { allowed: false, reason: 'Policy solana.maxDailySpendLamports is not a valid integer string.', policyDigest: digest };
            }
            if (!intent.sim.ok || intent.sim.lamportDelta === undefined) {
                return {
                    allowed: false,
 ...
 */
export function checkTronPolicy(intent, policy = loadPolicy(), spend) {
    const digest = policyDigest(policy);
    if (policy.killSwitch) {
        return { allowed: false, reason: 'Policy kill switch is active. Edit policy.json (set killSwitch: false) to allow operations.', policyDigest: digest };
    }
    ...
    if (tron.maxTxSun !== undefined) {
        let max;
        try {
            max = BigInt(tron.maxTxSun);
        }
        catch {
            return { allowed: false, reason: 'Policy tron.maxTxSun is not a valid integer string.', policyDigest: digest };
        }
    ...
    if (tron.maxFeeLimitSun !== undefined) {
        let max;
        try {
            max = BigInt(tron.maxFeeLimitSun);
        }
        catch {
            return { allowed: false, reason: 'Policy tron.maxFeeLimitSun is not a valid integer string.', policyDigest: digest };
        }
    ...
    if (hasVelocityCap) {
        ...
        if (tron.maxDailySpendSun !== undefined) {
            let maxSpend;
            try {
                maxSpend = BigInt(tron.maxDailySpendSun);
            }
            catch {
                return { allowed: false, reason: 'Policy tron.maxDailySpendSun is not a valid integer string.', policyDigest: digest };
            }
 ...
 */
export function checkErc4337Gate(intent, policy = loadPolicy()) {
    const digest = policyDigest(policy);
    if (policy.killSwitch) {
        return { allowed: false, reason: 'Policy kill switch is active. Edit policy.json (set killSwitch: false) to allow operations.', policyDigest: digest };
    }
...
export function validatePolicyInput(input) {
    ...
    for (const k of Object.keys(o)) {
        if (!ALLOWED_POLICY_FIELDS.has(k)) {
            return { ok: false, error: `Unknown policy field: ${k}` };
        }
    }
    if (o.version !== 1)
        return { ok: false, error: 'version must be 1' };
    if (typeof o.killSwitch !== 'boolean')
        return { ok: false, error: 'killSwitch must be true or false' };
    ...
    if (o.allowedChains !== undefined) {
        if (!Array.isArray(o.allowedChains))
            return { ok: false, error: 'allowedChains must be an array of integers' };
    ...
    for (const listField of ['allowedToAddresses', 'blockedToAddresses']) {
        const v = o[listField];
        if (v !== undefined) {
            if (!Array.isArray(v))
                return { ok: false, error: `${listField} must be an array of addresses` };
            for (const a of v) {
    ...
    if (o.maxTxValueWei !== undefined) {
        if (typeof o.maxTxValueWei !== 'string')
            return { ok: false, error: 'maxTxValueWei must be a string (to preserve precision)' };
        try {
            BigInt(o.maxTxValueWei);
        }
        catch {
            return { ok: false, error: 'maxTxValueWei must be a valid integer string' };
        }
    }
    if (o.maxTxGas !== undefined) {
        if (typeof o.maxTxGas !== 'string')
            return { ok: false, error: 'maxTxGas must be a string' };
        try {
            BigInt(o.maxTxGas);
        }
        catch {
            return { ok: false, error: 'maxTxGas must be a valid integer string' };
        }
    }
    if (o.maxDailySpendWei !== undefined) {
        if (typeof o.maxDailySpendWei !== 'string')
            return { ok: false, error: 'maxDailySpendWei must be a string (to preserve precision)' };
        try {
            BigInt(o.maxDailySpendWei);
        }
        catch {
            return { ok: false, error: 'maxDailySpendWei must be a valid integer string' };
        }
    }
    ...
    if (o.blockedSelectors !== undefined) {
        if (!Array.isArray(o.blockedSelectors))
            return { ok: false, error: 'blockedSelectors must be an array' };
    ...
    if (o.erc4337 !== undefined) {
        ...
        for (const k of Object.keys(e4)) {
            if (!E4_FIELDS.has(k))
                return { ok: false, error: `Unknown erc4337 policy field: ${k}` };
        }
        if (typeof e4.enabled !== 'boolean')
            return { ok: false, error: 'erc4337.enabled must be true or false' };
        if (e4.allowedAccounts !== undefined) {
            if (!Array.isArray(e4.allowedAccounts))
                return { ok: false, error: 'erc4337.allowedAccounts must be an array of addresses' };
            for (const a of e4.allowedAccounts) {
                if (typeof a !== 'string' || !HEX_ADDR_RE.test(a))
                    return { ok: false, error: `erc4337.allowedAccounts entries must be 0x addresses, got ${a}` };
            }
        }
        if (e4.allowedBundlerHosts !== undefined) {
            if (!Array.isArray(e4.allowedBundlerHosts))
                return { ok: false, error: 'erc4337.allowedBundlerHosts must be an array of hostnames' };
            for (const h of e4.allowedBundlerHosts) {
                if (typeof h !== 'string' || !/^[a-z0-9.-]+$/i.test(h))
                    return { ok: false, error: `erc4337.allowedBundlerHosts entries must be bare hostnames, got ${h}` };
            }
        }
    }
    if (o.solana !== undefined) {
        ...
        for (const k of Object.keys(sol)) {
            if (!SOLANA_FIELDS.has(k))
                return { ok: false, error: `Unknown solana policy field: ${k}` };
        }
        if (typeof sol.enabled !== 'boolean')
            return { ok: false, error: 'solana.enabled must be true or false' };
        for (const listField of ['allowedPrograms', 'blockedPrograms']) {
            const v = sol[listField];
            if (v !== undefined) {
                if (!Array.isArray(v))
                    return { ok: false, error: `solana.${listField} must be an array of base58 program ids` };
                for (const p of v) {
        ...
        for (const lamportField of ['maxTxLamports', 'maxDailySpendLamports']) {
            const v = sol[lamportField];
            if (v !== undefined) {
                if (typeof v !== 'string')
                    return { ok: false, error: `solana.${lamportField} must be a string (to preserve precision)` };
                try {
                    BigInt(v);
                }
                catch {
                    return { ok: false, error: `solana.${lamportField} must be a valid integer string` };
                }
            }
        }
    ...
    if (o.tron !== undefined) {
        ...
        for (const k of Object.keys(tron)) {
            if (!TRON_FIELDS.has(k))
                return { ok: false, error: `Unknown tron policy field: ${k}` };
        }
        if (typeof tron.enabled !== 'boolean')
            return { ok: false, error: 'tron.enabled must be true or false' };
        const TRON_ADDR_RE = /^T[1-9A-HJ-NP-Za-km-z]{33}$/;
        for (const listField of ['allowedContracts', 'blockedContracts']) {
            const v = tron[listField];
            if (v !== undefined) {
                if (!Array.isArray(v))
                    return { ok: false, error: `tron.${listField} must be an array of base58 Tron addresses` };
                for (const a of v) {
        ...
        for (const sunField of ['maxTxSun', 'maxDailySpendSun', 'maxFeeLimitSun']) {
            const v = tron[sunField];
            if (v !== undefined) {
                if (typeof v !== 'string')
                    return { ok: false, error: `tron.${sunField} must be a string (to preserve precision)` };
                try {
                    BigInt(v);
                }
                catch {
                    return { ok: false, error: `tron.${sunField} must be a valid integer string` };
                }
            }
        }
    ...
    const policy = {
        ...
        allowedChains: o.allowedChains,
        allowedToAddresses: o.allowedToAddresses,
        blockedToAddresses: o.blockedToAddresses,
        maxTxValueWei: o.maxTxValueWei,
        maxTxGas: o.maxTxGas,
        maxDailySpendWei: o.maxDailySpendWei,
        maxDailyTxCount: o.maxDailyTxCount,
        blockedSelectors: o.blockedSelectors,
        requireMemo: o.requireMemo,
        solana: o.solana,
        tron: o.tron,
        erc4337: o.erc4337,
        notes: o.notes,
