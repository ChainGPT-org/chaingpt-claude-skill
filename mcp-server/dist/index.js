...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name.startsWith('chaingpt_chat'))
            return await handleChatTool(name, args);
        if (name.startsWith('chaingpt_nft'))
            return await handleNftTool(name, args);
        if (name.startsWith('chaingpt_audit'))
            return await handleAuditTool(name, args);
        if (name.startsWith('chaingpt_generate'))
        ...
        if (name.startsWith('chaingpt_research'))
            return await handleResearchTool(name, args);
        if (name.startsWith('chaingpt_risk'))
            return await handleRiskTool(name, args);
        if (name.startsWith('chaingpt_onchain'))
            return await handleOnchainTool(name, args);
        if (name.startsWith('chaingpt_intel'))
            return await handleIntelTool(name, args);
        if (name.startsWith('chaingpt_deploy'))
            return await handleDeployTool(name, args);
        if (name.startsWith('chaingpt_dex_1inch') || name.startsWith('chaingpt_dex_cow'))
            return await handleAggregatorTool(name, args);
        if (name.startsWith('chaingpt_dex'))
            return await handleDexTool(name, args);
        if (name.startsWith('chaingpt_defi_pendle') || name.startsWith('chaingpt_defi_morpho'))
            return await handleYieldTool(name, args);
        ...
        if (name.startsWith('chaingpt_x402'))
            return await handleX402Tool(name, args);
        if (name.startsWith('chaingpt_base_'))
            return await handleBaseTool(name, args);
        if (name.startsWith('chaingpt_miniapp'))
            return await handleMiniappTool(name, args);
        if (name.startsWith('chaingpt_erc8004'))
            return await handleErc8004Tool(name, args);
        if (name.startsWith('chaingpt_defi'))
            return await handleDefiTool(name, args);
        if (name.startsWith('chaingpt_hl'))
            return await handleHyperliquidTool(name, args);
        if (name.startsWith('chaingpt_pm'))
            return await handlePolymarketTool(name, args);
        // ORDER: sub-prefixes must route before the generic agent-wallet prefix.
        if (name.startsWith('chaingpt_agent_wallet_4337'))
            return await handleAgentWallet4337Tool(name, args);
        if (name.startsWith('chaingpt_agent_wallet_solana'))
            return await handleAgentWalletSolanaTool(name, args);
        if (name.startsWith('chaingpt_agent_wallet_tron'))
            return await handleAgentWalletTronTool(name, args);
        if (name.startsWith('chaingpt_agent_wallet'))
            return await handleAgentWalletTool(name, args);
        if (name.startsWith('chaingpt_dashboard'))
            return await handleDashboardTool(name, args);
        if (name === 'chaingpt_strategy_save_plan' || name === 'chaingpt_strategy_load_plan' || name === 'chaingpt_strategy_list_plans' || name === 'chaingpt_strategy_delete_plan' || name === 'chaingpt_strategy_due_steps' || name === 'chaingpt_strategy_mark_step')
            return await handlePlanTool(name, args);
        if (name.startsWith('chaingpt_strategy') || name.startsWith('chaingpt_backtest'))
            return await handleStrategyTool(name, args);
        if (name.startsWith('chaingpt_bridge'))
            return await handleBridgeTool(name, args);
        if (name.startsWith('chaingpt_drift'))
            return await handleDriftTool(name, args);
        if (name.startsWith('chaingpt_portfolio'))
            return await handlePortfolioTool(name, args);
        if (name.startsWith('chaingpt_aa_session'))
            return await handleAaSessionTool(name, args);
        if (name.startsWith('chaingpt_aa_'))
            return await handleAaTool(name, args);
        if (name.startsWith('chaingpt_solana'))
            return await handleSolanaTool(name, args);
        if (name.startsWith('chaingpt_tron'))
            return await handleTronTool(name, args);
        if (name.startsWith('chaingpt_'))
            return await handleUtilTool(name, args);
        return {
