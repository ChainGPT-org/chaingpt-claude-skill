...
const ERC8004_CHAINS: Record<string, number> = {
...
export const erc8004Tools: Tool[] = [
  {
    name: 'chaingpt_erc8004_resolve_agent',
    description:
      'Resolve an ERC-8004 Trustless Agent by its agentId. Reads ownerOf + tokenURI from the Identity Registry ' +
  ...
  },
  {
    ...
    description:
      'Return the canonical ERC-8004 registry addresses (Identity + Reputation) and the chains they are deployed on. ' +
  ...
  },
  {
    ...
    description:
      'Generate an ERC-8004 agent registration card (the JSON served at the agent\'s tokenURI / ' +
...
function clientFor(chainKey: string) {
  ...
  if (endpoints.length === 0) endpoints.push('https://mainnet.base.org');
...
async function loadAgentCard(tokenURI: string): Promise<any | null> {
  try {
    ...
    if (/^https?:\/\//.test(url)) return await httpJson<any>(url, { method: 'GET' });
  } catch { /* ignore */ }
  return null;
}
...
async function handleResolve(args: any): Promise<string> {
  ...
  if (!(chainKey in ERC8004_CHAINS)) throw new Error(`Unknown chain "${chainKey}". One of: ${Object.keys(ERC8004_CHAINS).join(', ')}.`);
  const agentId = BigInt(String(args.agentId));
  const client = clientFor(chainKey);
  let owner: Address;
  try {
    owner = await client.readContract({ address: ERC8004.identity as Address, abi: IDENTITY_ABI, functionName: 'ownerOf', args: [agentId] }) as Address;
  } catch (e: any) {
  ...
  let tokenURI = '';
  try { tokenURI = await client.readContract({ address: ERC8004.identity as Address, abi: IDENTITY_ABI, functionName: 'tokenURI', args: [agentId] }) as string; } catch { /* */ }
  ...
  if (card) {
    lines.push(
      ...
      `active:      ${card.active ?? '?'}`,
      `x402:        ${card.x402Support ? 'yes' : 'no'}`,
      `trust:       ${Array.isArray(card.supportedTrust) ? card.supportedTrust.join(', ') : '(none)'}`,
      `services:    ${Array.isArray(card.services) ? card.services.length : 0}`,
    );
  } else if (tokenURI) {
...
function handleRegistries(): string {
  return [
...
function handleAgentCard(args: any): string {
  const chainKey = String(args.chain ?? 'base');
...
export async function handleErc8004Tool(name: string, args: any): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  let text: string;
  const a = args ?? {};
  try {
    if (name === 'chaingpt_erc8004_resolve_agent') text = await handleResolve(a);
    else if (name === 'chaingpt_erc8004_registries') text = handleRegistries();
    else if (name === 'chaingpt_erc8004_agentcard') text = handleAgentCard(a);
    else throw new Error(`Unknown ERC-8004 tool: ${name}`);
  } catch (e: any) {
