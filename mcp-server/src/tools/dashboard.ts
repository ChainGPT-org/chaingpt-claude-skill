...
export const dashboardTools: Tool[] = [
  {
    ...
    description:
      'Boot the marketplace dashboard — a localhost web UI with five read-only panels (Overview, Skills, ' +
...
const sessions = new Map<string, number>(); // sid -> expiry ms
...
function provisionAdminToken(): string {
...
function readSkillFrontmatter(skillDir: string): { name: string; description: string } | null {
...
function listSkills(): SkillCard[] {
  if (!existsSync(SKILLS_DIR)) return [];
  const out: SkillCard[] = [];
  let entries: string[] = [];
  try { entries = readdirSync(SKILLS_DIR); } catch { return []; }
...
// ── Activity (optional — only if agent-wallet has run) ─────────────
...
function readActivityRows(limit = 50): ActivityRow[] {
...
interface WalletData {
  ...
  policy: {
    ...
    maxTxValueWei: string | null;
    maxGasWei: string | null;
...
function getWallet(): WalletData {
  const keystore = readJsonSafe<{ address?: string }>(AGENT_KEYSTORE);
  const policy = readJsonSafe<Record<string, unknown>>(AGENT_POLICY);
  const tokens = readJsonSafe<Array<unknown>>(AGENT_TOKENS) ?? [];
  const chains = readJsonSafe<Array<unknown>>(AGENT_CUSTOM_CHAINS) ?? [];

  const chains_ = (policy?.allowedChains as unknown[] | undefined) ?? [];
  const allowedAddr = (policy?.allowedToAddresses as unknown[] | undefined) ?? [];
  const blockedAddr = (policy?.blockedToAddresses as unknown[] | undefined) ?? [];
  ...
  return {
    ...
    walletUiHint:
      'For full wallet management — balances across every chain, policy editing forms with kill switch, ' +
...
interface OverviewData {
  ...
  skillCount: number;
  agentWalletInitialized: boolean;
  agentWalletUiHint: string;
  homepage: string;
}

function getOverview(): OverviewData {
  const pluginPkg = readJsonSafe<Record<string, unknown>>(PLUGIN_JSON) || {};
  const marketplacePkg = readJsonSafe<{ plugins?: Array<{ version?: string }> }>(MARKETPLACE_JSON) || {};
  const mcpPkg = readJsonSafe<Record<string, unknown>>(MCP_SERVER_PKG_JSON) || {};
  ...
  return async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    try {
      ...
      if (url.startsWith('/dashboard/api/')) {
        if (!authed) { jsonResponse(res, 401, { error: 'unauthorized' }); return; }
        if (url === '/dashboard/api/overview') { jsonResponse(res, 200, getOverview()); return; }
        if (url === '/dashboard/api/wallet')    { jsonResponse(res, 200, getWallet()); return; }
        if (url === '/dashboard/api/skills')   { jsonResponse(res, 200, listSkills()); return; }
        if (url === '/dashboard/api/activity') { jsonResponse(res, 200, readActivityRows(50)); return; }
        if (url === '/dashboard/api/health')   { jsonResponse(res, 200, getHealth()); return; }
...
// ── Server lifecycle ───────────────────────────────────────────────
async function startServer(port: number): Promise<{ url: string; token: string; alreadyRunning: boolean }> {
...
export async function handleDashboardTool(name: string, args: unknown) {
  if (name !== 'chaingpt_dashboard_serve') {
    return { content: [{ type: 'text' as const, text: `Unknown dashboard tool: ${name}` }], isError: true };
  }
  const port = Number((args as { port?: number } | undefined)?.port) || 8788;
