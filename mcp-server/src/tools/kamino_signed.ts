...
export const kaminoSignedTools: Tool[] = [
  {
    name: 'chaingpt_defi_kamino_deposit_tx',
    description:
      'Build an UNSIGNED Kamino Lending deposit transaction (Solana mainnet, custody-free). Lends/supplies a token ' +
  ...
  },
  {
    ...
    description:
      'Build an UNSIGNED Kamino Lending withdraw transaction (Solana mainnet, custody-free). Withdraws a previously ' +
...
function toBaseUnits(human: string, decimals: number): string {
  const s = human.trim();
  if (!/^\d+(\.\d+)?$/.test(s)) throw new Error(`amount "${human}" is not a valid non-negative decimal number.`);
...
async function buildKaminoAction(kind: 'deposit' | 'withdraw', args: any): Promise<string> {
  ...
  if (!args.symbol && !args.mint) throw new Error('Provide either symbol (e.g. "USDC") or mint to identify the reserve.');
  ...
  const { KaminoMarket, KaminoAction, VanillaObligation } = klend as any;
  ...
  if (!reserve) {
    throw new Error(
      `No Kamino reserve for ${args.mint ? `mint ${args.mint}` : `symbol "${args.symbol}"`} in market ${marketAddress.toBase58()}. ` +
      `Check it against app.kamino.finance (symbol must match exactly), or pass a different market.`,
    );
  }
  const mint: PublicKey = reserve.getLiquidityMint?.() ?? new PublicKey(reserve.state.liquidity.mintPubkey);

  // Convert human amount -> base units using the on-chain mint decimals.
  const { decimals } = await fetchMintInfo(conn, mint);
  const baseAmount = withdrawAll ? U64_MAX : toBaseUnits(amount as string, decimals);
  ...
  if (ixs.length === 0) throw new Error('Kamino SDK produced no instructions for this action.');

  const luts: AddressLookupTableAccount[] = action.lookupTableAccounts ?? [];
  ...
  try {
    ...
    simText = formatSimResult(sim as any, 'Kamino');
  } catch (e: any) {
...
export async function handleKaminoSignedTool(name: string, args: any): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  let text: string;
  const safeArgs = args ?? {};
  try {
    if (name === 'chaingpt_defi_kamino_deposit_tx') text = await buildKaminoAction('deposit', safeArgs);
    else if (name === 'chaingpt_defi_kamino_withdraw_tx') text = await buildKaminoAction('withdraw', safeArgs);
    else throw new Error(`Unknown Kamino signed tool: ${name}`);
  } catch (e: any) {
