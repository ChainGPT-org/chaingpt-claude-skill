...
import { loadPolicy, policyPath, checkTronPolicy, type TronTxIntent } from '../lib/agent-policy.js';
...
export const agentWalletTronTools: Tool[] = [
  {
    name: 'chaingpt_agent_wallet_tron_address',
    description:
      "Show the agent's Tron address (derived from the SAME key as the EVM agent wallet — no separate keystore) and its TRX balance. Fund this address to give the agent Tron working capital. 0 ChainGPT credits.",
    inputSchema: {
      type: 'object' as const,
      properties: { network: { type: 'string', enum: NETWORK_ENUM as unknown as string[], default: 'mainnet' } },
      required: [],
    },
  },
  {
...
export async function handleAgentWalletTronTool(
  name: string,
  args: Record<string, unknown> | undefined,
): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  const a = args ?? {};
  const network = (String(a.network ?? 'mainnet')) as TronNetwork;

  try {
    if (name === 'chaingpt_agent_wallet_tron_address') {
      const file = readKeystoreFile();
      if (!file) return text('Agent wallet not initialized. Call chaingpt_agent_wallet_init first (the EVM keystore is shared with Tron).');
    ...
    if (name === 'chaingpt_agent_wallet_tron_sign_and_send') {
      ...
      let callValueSun = 0n;
      let policyTo: string;
      let policyValueSun: bigint;
      let summary: string[];
      let unsigned;

      if (kind === 'trx_transfer') {
        const to = String(a.to ?? '');
        if (!isTronAddress(to)) return text(`Invalid recipient address: "${to}"`);
        ...
        const FEE_BUFFER_SUN = 1_000_000n; // reserve ~1 TRX for bandwidth/energy burn
        if (bal < amountSun + FEE_BUFFER_SUN) return text(`⛔ Refused: agent balance ${formatUnits(bal, TRX_DECIMALS)} TRX is less than the ${formatUnits(amountSun, TRX_DECIMALS)} TRX transfer plus a ~1 TRX fee buffer.`);
        unsigned = await buildTrxTransfer(network, { ownerBase58: agentTron, toBase58: to, amountSun });
        policyTo = to; policyValueSun = amountSun;
        ...
        if (!isTronAddress(tokenAddr)) return text(`Invalid token: "${a.token}"`);
        ...
        policyTo = tokenAddr; policyValueSun = 0n;
        ...
        if (!isTronAddress(c)) return text(`Invalid contract address: "${c}"`);
        assertNotPoisoned(c);
        functionSelector = String(a.functionSelector ?? '');
        if (!/^[A-Za-z_][\w]*\(.*\)$/.test(functionSelector)) return text(`Invalid functionSelector: "${functionSelector}" (e.g. "transfer(address,uint256)").`);
        parameter = a.parameter ? String(a.parameter).replace(/^0x/, '') : '';
        if (parameter && !/^[0-9a-fA-F]*$/.test(parameter)) return text('parameter must be hex (no 0x).');
        ...
        policyTo = c; policyValueSun = callValueSun;
      ...
      const intent: TronTxIntent = { owner: agentTron, to: policyTo, valueSun: policyValueSun, feeLimitSun: contract ? feeLimitSun : 0n, memo };
