...
export const x402Tools: Tool[] = [
  {
    ...
    description:
      'Decode an x402 challenge or header into human-readable terms. Accepts either a 402 response BODY (JSON with ' +
  ...
  },
  {
    ...
    description:
      'Build the UNSIGNED EIP-712 authorization for an x402 "exact" payment (EIP-3009 transferWithAuthorization, e.g. ' +
  ...
  },
  {
    ...
    description:
      'Call an x402 facilitator endpoint over HTTP: "supported" (list chains/tokens/schemes), "verify" (check a ' +
  ...
  },
  {
    ...
    description:
      'Server-side helper: build a PaymentRequirements object + a complete 402 response body to monetize your own ' +
...
function resolveToken(network: string, symbol: string) {
  const key = `${network}:${symbol.toUpperCase()}`;
  const token = X402_TOKENS[key];
  if (!token) {
    throw new Error(
      `No known EIP-3009 token for "${key}". Known: ${Object.keys(X402_TOKENS).join(', ')}. ` +
...
function describeRequirements(r: PaymentRequirements): string {
  const sym = (r.extra as any)?.name ? '' : '';
...
async function handleDecode(args: any): Promise<string> {
  if (args.header) {
    const decoded = decodeBase64Header<any>(String(args.header));
  ...
  if (accepts.length === 0) throw new Error('No PaymentRequirements found. Pass the 402 response `body` (with accepts[]) or a base64 `header`.');
  return [
...
async function handleBuildPayment(args: any): Promise<string> {
  ...
  if (!isAddress(from)) throw new Error(`from is not a valid EVM address: ${from}`);

  const req: PaymentRequirements | undefined = args.requirements;
  ...
  let token: { address: string; name: string; version: string; decimals: number };
  if (req?.asset && (req.extra as any)?.name && (req.extra as any)?.version) {
    const known = Object.values(X402_TOKENS).find((t) => t.address.toLowerCase() === req.asset.toLowerCase());
    token = { address: req.asset, name: (req.extra as any).name, version: (req.extra as any).version, decimals: known?.decimals ?? 6 };
  } else {
    token = resolveToken(network, String(args.symbol ?? 'USDC'));
  }

  const payTo = String(req?.payTo ?? args.payTo ?? '');
  if (!isAddress(payTo)) throw new Error(`payTo (recipient) is not a valid EVM address: ${payTo}`);

  let atomic: string;
  if (req?.maxAmountRequired) atomic = req.maxAmountRequired;
  else if (args.maxAmountRequired) atomic = String(args.maxAmountRequired);
  else if (args.amount != null) {
    const [w, f = ''] = String(args.amount).split('.');
    if (f.length > token.decimals) throw new Error(`amount has more decimals than the token (${token.decimals}).`);
  ...
  if (args.signature) {
    const payload: PaymentPayload = {
      ...
      scheme: 'exact',
      network,
      payload: { signature: String(args.signature) as Hex, authorization },
    };
...
async function handleFacilitator(args: any): Promise<string> {
  const base = String(args.facilitatorUrl).replace(/\/+$/, '');
  const action = String(args.action);
  if (action === 'supported') {
    const res = await httpJson<any>(`${base}/supported`, { method: 'GET' });
  ...
  if (action === 'verify' || action === 'settle') {
    ...
    const res = await httpJson<any>(`${base}/${action}`, {
      method: 'POST',
...
async function handleCreateRequirements(args: any): Promise<string> {
  const network = String(args.network);
  const chainId = chainIdForNetwork(network);
  const token = resolveToken(network, String(args.symbol ?? 'USDC'));
  const payTo = String(args.payTo);
  if (!isAddress(payTo)) throw new Error(`payTo is not a valid EVM address: ${payTo}`);
  const [w, f = ''] = String(args.amount).split('.');
  if (f.length > token.decimals) throw new Error(`amount has more decimals than the token (${token.decimals}).`);
...
export async function handleX402Tool(name: string, args: any): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  let text: string;
  const a = args ?? {};
  try {
