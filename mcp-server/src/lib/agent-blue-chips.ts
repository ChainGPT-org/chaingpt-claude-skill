 ...
 */
export const BLUE_CHIPS: Record<string, BlueChipToken[]> = {
  ethereum: [
...
export function getBlueChipsForChain(chain: string): BlueChipToken[] {
  return BLUE_CHIPS[chain.toLowerCase()] ?? [];
}

export function allBlueChips(): Array<BlueChipToken & { chain: string }> {
  const out: Array<BlueChipToken & { chain: string }> = [];
  for (const [chain, tokens] of Object.entries(BLUE_CHIPS)) {
    for (const t of tokens) out.push({ ...t, chain });
  }
  return out;
}
