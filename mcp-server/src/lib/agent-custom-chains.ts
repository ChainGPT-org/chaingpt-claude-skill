...
export interface CustomChain {
  ...
  chainId: number;
  name: string;
  native: string;        // e.g. "ETH"
  rpcUrl: string;        // single RPC; we'll also accept additional fallbacks
  rpcFallbacks?: string[];
  explorer?: string;     // e.g. "https://yourscan.io"
  addedAt: string;
}
...
export function addCustomChain(input: Omit<CustomChain, 'addedAt'>): { ok: true; chains: CustomChain[] } | { ok: false; error: string } {
  if (!SLUG_RE.test(input.slug)) return { ok: false, error: `slug must be lowercase letters/digits/dashes, 2-31 chars (got "${input.slug}")` };
  if (CHAINS[input.slug]) return { ok: false, error: `slug "${input.slug}" collides with a built-in chain` };
  if (!Number.isInteger(input.chainId) || input.chainId < 1) return { ok: false, error: `chainId must be a positive integer (got ${input.chainId})` };
  if (!input.name.trim()) return { ok: false, error: 'name is required' };
  if (!input.native.trim()) return { ok: false, error: 'native symbol is required' };
  if (!HTTPS_RE.test(input.rpcUrl)) return { ok: false, error: 'rpcUrl must be a valid http(s) URL' };
  if (input.rpcFallbacks?.some((u) => !HTTPS_RE.test(u))) return { ok: false, error: 'rpcFallbacks entries must all be http(s) URLs' };
  if (input.explorer && !HTTPS_RE.test(input.explorer)) return { ok: false, error: 'explorer must be a valid http(s) URL' };

  // Collisions with existing custom chains
  const existing = loadCustomChains();
  if (existing.some((c) => c.slug === input.slug)) return { ok: false, error: `slug "${input.slug}" is already in use` };
  if (existing.some((c) => c.chainId === input.chainId)) return { ok: false, error: `chainId ${input.chainId} is already in use` };
  // Collision with built-in chainIds
  for (const c of Object.values(CHAINS)) {
    if (c.chainId === input.chainId) return { ok: false, error: `chainId ${input.chainId} collides with built-in ${c.slug}` };
  }

  const next: CustomChain = { ...input, addedAt: new Date().toISOString() };
 ...
 */
export function mergedChains(): Record<string, ChainInfo> {
  const merged: Record<string, ChainInfo> = { ...CHAINS };
...
export function resolveChainWithCustom(input: string | number | undefined | null): ChainInfo | undefined {
  if (input === undefined || input === null || input === '') return undefined;
  const s = String(input).trim().toLowerCase();
  const merged = mergedChains();
  if (merged[s]) return merged[s];
