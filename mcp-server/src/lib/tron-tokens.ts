...
export const TRON_TOKENS: Record<string, TronToken> = {
 ...
 */
export const TRON_POISONED_ADDRESSES: ReadonlySet<string> = new Set([
...
export const TRON_DEFI = {
  ...
  justlend: {
    ...
    } as Record<string, { jToken: string; underlying: string | null; isCEther: boolean }>,
  },
} as const;
...
export function assertNotPoisoned(address: string): void {
 ...
 */
export function resolveTronToken(symbolOrAddress: string): TronToken | undefined {
  if (typeof symbolOrAddress !== 'string' || symbolOrAddress.trim() === '') return undefined;
  const s = symbolOrAddress.trim();

  // Symbol match (case-insensitive; handle the stUSDT/STUSDT key).
  const upper = s.toUpperCase();
  if (TRON_TOKENS[upper]) return TRON_TOKENS[upper];

  // Address match.
  if (isTronAddress(s)) {
    assertNotPoisoned(s);
    for (const t of Object.values(TRON_TOKENS)) {
      if (t.address === s) return t;
    }
  }
  return undefined;
}
