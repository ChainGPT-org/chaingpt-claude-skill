...
import { type TronNetwork, type UnsignedTronTx, triggerConstantContract } from './tron.js';
 ...
 */
export async function sunswapAmountsOut(network: TronNetwork, amountIn: bigint, pathBase58: string[]): Promise<bigint[]> {
  if (pathBase58.length < 2) throw new Error('swap path needs at least 2 tokens');
  ...
  if (!res) throw new Error('SunSwap getAmountsOut returned no result (no V2 pool for this path?)');
  const [amounts] = decodeAbiParameters([{ type: 'uint256[]' }], `0x${res}`);
  return (amounts as readonly bigint[]).map((a) => a);
}

// ── JustLend (Compound-V2 fork) ──────────────────────────────────────
export type JustlendMarket = 'TRX' | 'USDT' | 'USDD';
export type JustlendAction = 'approve' | 'supply' | 'withdraw' | 'borrow' | 'repay';

function market(symbol: string): { jToken: string; underlying: string | null; isCEther: boolean } {
  const m = TRON_DEFI.justlend.markets[symbol.toUpperCase()];
  if (!m) throw new Error(`unknown JustLend market "${symbol}" (supported: ${Object.keys(TRON_DEFI.justlend.markets).join(', ')})`);
  return m;
}

/** Account-wide liquidity (USD, 1e18). shortfall>0 ⇒ liquidatable. error must be 0. */
export async function justlendAccountLiquidity(
  network: TronNetwork,
  ownerBase58: string,
  ...
  const res = r.constant_result?.[0];
  if (!res) throw new Error('JustLend getAccountLiquidity returned no result');
  const [error, liquidity, shortfall] = decodeAbiParameters(
    [{ type: 'uint256' }, { type: 'uint256' }, { type: 'uint256' }],
    `0x${res}`,
  );
  return { error: error as bigint, liquidity: liquidity as bigint, shortfall: shortfall as bigint };
}

/** Per-market supplied + borrowed underlying balances (constant-call simulates the accruing reads). */
export async function justlendMarketBalances(
  network: TronNetwork,
  ownerBase58: string,
  symbol: JustlendMarket,
 ...
 */
export async function buildJustlendTx(
  network: TronNetwork,
  args: { ownerBase58: string; market: JustlendMarket; action: JustlendAction; amount: bigint; feeLimitSun?: bigint },
  ...
  switch (args.action) {
    case 'approve':
      if (isCEther || !underlying) throw new Error('the TRX market needs no approval');
