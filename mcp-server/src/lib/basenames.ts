...
import { createPublicClient, http, fallback, namehash, getAddress, type Address, type PublicClient } from 'viem';
...
export type BaseChainId = keyof typeof BASENAMES;
...
export function baseRpcEndpoints(chainId: BaseChainId): string[] {
  ...
  if (env) return [env, ...list];
  if (list.length === 0) return [chainId === 8453 ? 'https://mainnet.base.org' : 'https://sepolia.base.org'];
  return list;
}

export function basePublicClient(chainId: BaseChainId): PublicClient {
  const transports = baseRpcEndpoints(chainId).map((url) => http(url, { timeout: 8_000 }));
  return createPublicClient({ transport: transports.length === 1 ? transports[0] : fallback(transports) }) as PublicClient;
}

/** Normalize a basename: ensure it ends with `.base.eth`. Returns { full, label }. */
export function normalizeBasename(input: string): { full: string; label: string } {
  ...
  if (!label || label.includes('.')) throw new Error(`Invalid basename "${input}" — expected a single label like "alice" or "alice.base.eth".`);
  return { full, label };
}

export async function resolveForward(chainId: BaseChainId, name: string): Promise<string | null> {
  const { full } = normalizeBasename(name);
  const a = BASENAMES[chainId];
  const node = namehash(full);
  const resolver = await basePublicClient(chainId).readContract({ address: a.registry as Address, abi: REGISTRY_ABI, functionName: 'resolver', args: [node] }) as Address;
  if (!resolver || resolver === '0x0000000000000000000000000000000000000000') return null;
  const addr = await basePublicClient(chainId).readContract({ address: resolver, abi: RESOLVER_ABI, functionName: 'addr', args: [node] }) as Address;
  if (!addr || addr === '0x0000000000000000000000000000000000000000') return null;
  return getAddress(addr);
}

export async function resolveReverse(chainId: BaseChainId, address: string): Promise<string | null> {
  const a = BASENAMES[chainId];
  const client = basePublicClient(chainId);
  const rnode = await client.readContract({ address: a.reverseRegistrar as Address, abi: REVERSE_REGISTRAR_ABI, functionName: 'node', args: [getAddress(address)] }) as `0x${string}`;
  const resolver = await client.readContract({ address: a.registry as Address, abi: REGISTRY_ABI, functionName: 'resolver', args: [rnode] }) as Address;
  if (!resolver || resolver === '0x0000000000000000000000000000000000000000') return null;
  const name = await client.readContract({ address: resolver, abi: RESOLVER_ABI, functionName: 'name', args: [rnode] }) as string;
  return name || null;
}
