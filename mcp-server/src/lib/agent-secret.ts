...
export type KeychainBackend = 'macos-keychain' | 'libsecret' | null;
...
export function detectKeychainBackend(): KeychainBackend {
  ...
  if (process.platform === 'darwin') {
    if (commandExists('security')) return 'macos-keychain';
    return null;
  }
  if (process.platform === 'linux') {
    if (commandExists('secret-tool')) return 'libsecret';
    return null;
  }
...
function keychainSet(value: string): void {
  const backend = detectKeychainBackend();
  if (!backend) throw new Error('No OS keychain backend available on this host.');
  if (backend === 'macos-keychain') {
  ...
  if (backend === 'libsecret') {
    ...
    execFileSync(
      'secret-tool',
      ['store', '--label=ChainGPT MCP agent wallet passphrase', 'service', SERVICE, 'account', ACCOUNT],
 ...
 */
export function resolvePassphrase(): { value: string | null; source: SecretSource } {
 ...
 */
export function provisionKeychainPassphrase(): { value: string; source: 'keychain' } {
