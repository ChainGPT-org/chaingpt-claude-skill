...
[![MCP Tools](https://img.shields.io/badge/MCP_tools-134-blueviolet?style=flat-square)](#-mcp-server--134-tools)
...
[Quickstart](#-quickstart) · [Why this exists](#-why-this-exists) · [How it works](#-how-it-works) · [Security model](#-security-model) · [MCP tools](#-mcp-server--134-tools) · [Agent Wallet](#-the-agent-wallet-dashboard) · [vs alternatives](#-how-this-compares) · [Docs](https://docs.chaingpt.org/dev-docs-b2b-saas-api-and-sdk)
...
| **An agent builder** | `/agents` → `web3-researcher` / `defi-trader` | [MCP tools](#-mcp-server--134-tools) · `agents/` + `hooks/` · x402 + ERC-8004 skills |
...
This re‑scans installed plugins, spawns the ChainGPT MCP server over stdio, and registers all 23 sub‑skills + 134 MCP tools.
...
You should see `chaingpt` listed with **134 tools**.
...
└────────┬─────────┘  └──────────────────┘  └──────────────────────┘
         ...
         ▼
┌────────────────────────────────────────────────────────────────────────┐
│  MCP Server — 134 tools (the runtime)                                  │
│                                                                        │
...
The MCP server gives Claude **direct API and on‑chain access** — not just code generation. 134 tools across 15 categories. Detailed sections follow.
...
| `mcp-test` (vitest — handlers, policy gate, signing, schemas) | **391** | none |
...
|---|---|---|---|---|---|
| **Surface** | 134 MCP tools across 11 categories incl. perps + prediction markets + cross‑chain + agent wallet | Plugin per protocol (extensible) | EVM swap + on‑chain + Base‑native | Wallet + LLM bridge | Image gen + LLM marketplace |
...
**Where this wins:** breadth (134 tools), AI‑enriched composed tools (the DexScreener + GoPlus + News + AI signal combo), mainnet safety guard rails, and the agent‑wallet admin dashboard.
...
├── mcp-server/                       # MCP server — 134 tools, 391 vitest cases
...
- [x] **134 MCP tools** across ChainGPT AI, EVM + Solana DEX (OpenOcean · 1inch v6 · CoW · Jupiter), DeFi (Aave · Lido · EigenLayer · Pendle · Morpho), perps (Hyperliquid · Drift), prediction markets (Polymarket), cross‑chain (Across), Solana lending (Marginfi · Kamino), multi‑protocol portfolio snapshot, strategy plan persistence + backtest
...
- [x] **Unified test harness** — `./scripts/test-all.sh` runs six layers. 391 vitest + 26 mock + 159 validate + 39 live‑API cases
